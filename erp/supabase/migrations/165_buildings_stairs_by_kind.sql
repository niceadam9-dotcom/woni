...
-- ═══ 1단계 · DDL ═══════════════════════════════════════════════════════════
--  🚨 **여기까지 먼저 돌리고 커밋한다.** 아래 백필과 한 배치로 보내면 배치 전체가 한 트랜잭션이라
--    백필 한 줄이 터졌을 때 **이 ALTER까지 같이 롤백된다** — 그러면 「돌렸는데 컬럼이 없다」가 되어
--    무엇이 실패했는지 단서가 남지 않는다(2026-09-16에 실제로 그렇게 됐다).
--    단을 갈라 두면 실패가 어느 쪽인지 그 자리에서 보인다.
...
-- ═══ 2단계 · 백필 ══════════════════════════════════════════════════════════
--  🚨 **1단계가 커밋된 뒤에 돌린다.** 여기서는 위에서 만든 컬럼을 이름으로 지목하는데,
--    같은 배치로 보내면 풀러·플래너가 계획 단계에서 그 이름을 아직 못 찾을 수 있다
--    (`sections`는 JSONB가 맞으니 `->` 연산자는 범인이 아니다 — 2026-09-16에 확인했다).
-- ── 백필 ────────────────────────────────────────────────────────────────────
