 ...
 *
 *  ⚠ 감추는 것은 **표(tbody)뿐이고 머리줄은 남는다** — 개수가 거기서만 보인다.
 *    🚨 2026-09-11까지 이 자리의 이유는 「머리줄에 [+ 건물 등록]이 있고 그것이 2번째 동을 추가하는
 *      유일한 문이므로 표째 감추면 문이 함께 사라진다」였다. 그 버튼은 소방계획서_49 §10-2 ②로
 *      **의도적으로 제거됐다**(고객 1 : 건물 1 — 실측 다동 고객 0명). 그래서 그 논거는 더 이상
 *      성립하지 않는다. 이유가 바뀐 것을 적어 두지 않으면 다음 사람이 규칙을 잘못 되짚는다.
