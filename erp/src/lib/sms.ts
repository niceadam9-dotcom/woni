...
export async function sendInspectionSms(
   ...
   *
   *  **claim(④) 이후의 모든 종료 경로에서 불린다** — 성공만이 아니라 claim 실패·경합 중단에서도
   *  남겨야 한다. 중단됐다고 그 고객이 문자를 받은 것은 아니기 때문이다. 두 번 불려도 안전하도록
   *  한 번만 실행한다(중단 경로가 정상 경로와 겹칠 수 있다).
   *  이 기록 실패는 발송을 막지 않는다(보낼 대상과 무관) — 다만 삼키지 않고 로그로 표면화한다.
   *
   *  ⚠ **claim 이전의 조기 반환(①상한·①-b오타·①-c전건빈값·②리허설·②-b중복·③allowlist)은
   *    부르지 않는다** — 이 함수 선언 자체가 그 반환들보다 아래에 있어 구조적으로 부를 수 없다.
   *    의도된 경계다: 그 지점들은 **배치가 아예 시작되지 않은** 상태라 화면에 사유가 그대로 뜨고
   *    사용자가 고쳐서 다시 누른다. 거기서까지 no_phone 행을 남기면 '보내려다 만 흔적'이
   *    이력에 쌓여 진짜 미수신 기록을 묻는다.
   *    (종전 주석은 "모든 종료 경로"라고 적혀 있었다 — 코드보다 넓은 주장이라, 읽는 사람이
   *     ②-b 중복 차단에서도 기록이 남는다고 믿게 만든다. 2026-09-08 독립 판정에서 정정.) */
