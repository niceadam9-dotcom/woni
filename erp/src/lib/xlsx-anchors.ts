 ...
 *  삽입 기준점(소방계획서_46)이 **같은 상수**를 본다. 두 벌로 두면 한쪽만 갱신돼, 사진 대지가
...
export const ANCHORS: Anchor[] = [
  ...
  // 완료보고서 보고일 G25 (소방계획서_43 S4 / Q-3 확정) — 서식은 `=개요!G10+5`, 즉 **이행조치
  // 종료일 + 5일**이라는 근거 없는 추정이었다. PDF 11호는 오늘(또는 작성 패널 수기값)을 찍으므로
  // 같은 문서의 두 표면이 다른 날짜를 인쇄했다(D-4). 값은 `annexReportDateISO()` 단일 원천.
  // ⚠ dropFormula 필수 — 살려 두면 Excel 재계산이 `개요!G10+5`로 되돌린다(I20과 같은 함정).
  // ⚠ **이 칸에는 인접 라벨이 없다.** 시트에서 쓸 수 있는 고정 리터럴은 A24(법정 문구)뿐이라
  //   그것을 라벨로 삼는다 — 길지만 이 시트에서 유일하고(전 셀 덤프 실측), 서식이 밀리면
  //   자가치유가 A24를 다시 찾아 (열 +6, 행 +1) 오프셋으로 따라온다.
  {
    field: 'doneReportSerial', sheet: '완료보고서', cell: 'G25', labelCell: 'A24',
    label: '「소방시설 설치 및 안전관리에 관한 법률」 제23조제4항 및 같은 법 시행규칙 제23조제6항에 따라 위와 같이 소방시설등의 수리ㆍ교체ㆍ정비에 대한 이행완료 보고서를 제출합니다.',
    // 값이 없으면 서식 수식을 살려 둔다 — 허브 G9(이행조치 시작일)와 같은 판단이다: ERP가 모르는
    // 값은 서식의 기본값이 남는 편이 낫다. 실제로는 라우트가 늘 오늘 날짜를 주므로 빈 경우는
  ...
  // ── 현2 3-3 수계 개별사항 · 3-4 가스계 23칸 (Phase 4 / S9-1) ────────────────
  //
  // 라벨칸은 각 설비의 이름 칸이다(B24 옥내소화전 … B46 고체에어로졸). 설비 설치 마크
  // (A24·A27·…·A46)는 `현황!C12~C25` 수식이라 **앵커를 달지 않는다** — 이미 흐른다.
  // 3-4는 6개 가스 설비가 라벨 열을 나눠 갖고 본문은 한 블록이라, 각 줄에 **가장 가까운**
  // 라벨을 물린다(전부 같은 라벨로 묶으면 서식이 밀렸을 때 서로 구별되지 않는다).
  ...([
    ['s33IhLoc', 'C24', 'B24', '옥내소화전'], ['s33IhLoc2', 'C25', 'B24', '옥내소화전'],
    ['s33IhMax', 'C26', 'B24', '옥내소화전'],
    ['s33OhCount', 'C27', 'B27', '옥외소화전'],
    ['s33SpType', 'C28', 'B28', '스프링클러설비'], ['s33SpLoc', 'C29', 'B28', '스프링클러설비'],
    ['s33SsType', 'C30', 'B30', '간이\n스프링클러설비'], ['s33SsLoc', 'C31', 'B30', '간이\n스프링클러설비'],
    ['s33EsLoc', 'C32', 'B32', '화재\n조기진압용'], ['s33EsLoc2', 'C33', 'B32', '화재\n조기진압용'],
    ['s33WsLoc', 'C34', 'B34', '물분무\n소화설비'], ['s33WsLoc2', 'C35', 'B34', '물분무\n소화설비'],
    ['s33FoSystem', 'C36', 'B36', '포\n소화설비'], ['s33FoAgent', 'C37', 'B36', '포\n소화설비'],
    ['s33FoLoc', 'C38', 'B36', '포\n소화설비'],
    ['s34Discharge', 'C40', 'B40', '이산화탄소'],
    ['s34Loc', 'C41', 'B41', '할론'],
    ['s34Storage', 'C42', 'B42', '할로겐화합물\n및 불활성기체'],
    ['s34Qty', 'C43', 'B42', '할로겐화합물\n및 불활성기체'],
    ['s34Agent1', 'C44', 'B44', '분말'],
    ['s34Agent2', 'C45', 'B45', '강화액'],
    ['s34Agent3', 'C46', 'B46', '고체에어로졸'], ['s34Agent4', 'C47', 'B46', '고체에어로졸'],
  ] as const).map<Anchor>(([field, cell, labelCell, label]) => ({ field, sheet: '현2', cell, labelCell, label })),
  // ── 현3 3-5 경보설비 21칸 (Phase 4 / S9-1) ─────────────────────────────────
  // 설비 설치 마크 8칸(A3·A5·A8·A13·A15·A16·A19·A21)은 `현황!C26~C33` 수식이라 배선하지 않는다.
  ...([
    ['s35SdLoc', 'C3', 'B3', '단독\n경보형감지기'], ['s35SdPower', 'C4', 'B3', '단독\n경보형감지기'],
    ['s35EbType', 'C5', 'B5', '비상경보설비'], ['s35EbLoc', 'C6', 'B5', '비상경보설비'],
    ['s35EbPanel', 'C7', 'B5', '비상경보설비'],
    ['s35FdReceiver', 'C8', 'B8', '자동화재\n탐지설비'], ['s35FdMode', 'C9', 'B8', '자동화재\n탐지설비'],
    ['s35FdLoc', 'C10', 'B8', '자동화재\n탐지설비'], ['s35FdLoc2', 'C11', 'B8', '자동화재\n탐지설비'],
    ['s35FdDet', 'C12', 'B8', '자동화재\n탐지설비'],
    ['s35BcUsage', 'C13', 'B13', '비상방송설비'], ['s35BcAmp', 'C14', 'B13', '비상방송설비'],
    ['s35ArLoc', 'C15', 'B15', '자동화재\n속보설비'],
    ['s35ImMain', 'C16', 'B16', '통합감시시설'], ['s35ImSub', 'C17', 'B16', '통합감시시설'],
    ['s35ImNet', 'C18', 'B16', '통합감시시설'],
    ['s35LaLoc', 'C19', 'B19', '누전경보기'], ['s35LaGrade', 'C20', 'B19', '누전경보기'],
    ['s35GaForm', 'C21', 'B21', '가스누설\n경보기'], ['s35GaLoc', 'C22', 'B21', '가스누설\n경보기'],
    ['s35GaBreaker', 'C23', 'B21', '가스누설\n경보기'],
  ...
  ...([
    ['s36EvTypes1', 'C25', 'B25', '피난기구'], ['s36EvTypes2', 'C26', 'B25', '피난기구'],
    ['s36EvLoc', 'C27', 'B25', '피난기구'], ['s36EvLoc2', 'C28', 'B25', '피난기구'],
    ['s36ReTypes', 'C29', 'B29', '인명\n구조기구'], ['s36ReLoc', 'C30', 'B29', '인명\n구조기구'],
    ['s36ReLoc2', 'C31', 'B29', '인명\n구조기구'], ['s36ReUse1', 'C32', 'B29', '인명\n구조기구'],
    ['s36ReUse2', 'C33', 'B29', '인명\n구조기구'],
    ['s36GlTypes', 'C34', 'B34', '유도등'], ['s36GlLoc', 'C35', 'B34', '유도등'],
    ['s36ElLoc', 'C36', 'B36', '비상조명등'], ['s36ElPower', 'C37', 'B36', '비상조명등'],
    ['s36PlLoc', 'C38', 'B38', '휴대용\n 비상조명등'], ['s36PlPower', 'C39', 'B38', '휴대용\n 비상조명등'],
    ['s37WwPlace', 'C41', 'B41', '상수도\n소화용수'],
    ['s37WtKind', 'C42', 'B42', '소화수조'], ['s37WtPump', 'C43', 'B42', '소화수조'],
