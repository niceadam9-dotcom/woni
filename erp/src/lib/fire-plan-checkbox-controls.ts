 ...
 *
 *  🚨🚨 **자[尺]를 MS Excel에서 LibreOffice로 옮겼다.** 실측 기준선은 **16.13px**다
 *    (선두가 상자인 라벨 333개 중앙값 · 폭 1.52px). 경위는 `fire-plan-checkbox-offsets.ts`.
 *
 *  🚨🚨🚨 **그런데 그 기준선을 그대로 쓰지 않는다 — 3px 왼쪽으로 뺀다.**
 *
 *    상자 글리프에 **정확히** 겹쳐 놓았더니(16px), `□유`·`☐지휘통제팀`처럼 **상자 뒤에 공백이
 *    없는** 라벨에서 컨트롤 오른쪽과 글자 사이 여유가 **0~1px**이 됐다(136개 중 12개).
 *    내 렌더(LO 인쇄)에선 「닿음」이라 「겹침 0건」으로 통과했지만, 사용자 화면에선 **「겹침」**으로
 *    넘어갔다(2026-09-16 지적 image-8~11). 렌더러가 몇 px만 달라도 뒤집히는 자리였다.
 *
 *    ⭐ **정렬이 아니라 여유가 목표다.** 원래 상자 글자는 **바탕색으로 칠해 안 보이므로**
 *      컨트롤이 그 자리에 픽셀 단위로 맞을 이유가 없다 — 눈에 보이는 제약은 **「글자를 안 덮는다」**
 *      하나뿐이다. 글리프의 오른쪽 사이드베어링만큼 물러나면 그 제약이 넉넉해진다.
 *    ⚠ 왼쪽은 칸 여백이라 3px을 내줘도 테두리를 안 넘는다(컨트롤 왼쪽 끝 13px > 0).
 *    ⚠ 16으로 되돌리려거든 `_probe-cb-collide.mts`로 **여유 최솟값**을 먼저 볼 것.
 *      「겹친 것 0」만 보면 0px짜리가 숨는다 — 그게 사용자를 세 번 오게 한 이유다. */
export const TEXT_INSET_PX = 13
