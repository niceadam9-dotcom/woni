...
export function buildWorkbookValues(src: WorkbookSource): Map<string, CellValue> {
  ...
  // 불량에만 적용한 것)이다.
  //
  // 🚨🚨 **여기는 PDF 10호와 일부러 다르다 — 통일하지 말 것**(2026-09-10 사용자 결정).
  //   PDF 10호 7행(`annexPlanRows`)은 같은 날짜를 여러 번 인쇄하지 않으려고 일자 칸을
  //   **「결과참조」**로 바꿨다(날짜는 「이행조치 필요기간」 한 줄이 단독으로 싣는다). 그런데
  //   이 21칸(`계획서!K·P·O`)은 **엑셀 날짜 셀(serial)**이라 문자열을 넣으면 셀 서식과 어긋난다.
  //   그래서 **엑셀만 날짜를 유지**하기로 했다.
