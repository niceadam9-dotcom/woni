...
export function annexPlanRows(d: Report9Data): AnnexPlanRow[] {
  ...
  return DEFECT_GROUPS.map(group => {
    ...
    return {
      ...
      /* 🚨 2026-09-11 (같은 날 두 번째 정정) — **불량 유무와 무관하게 7행 전부** 기간을 싣는다.
         몇 시간 전 판(`hasDefect ? period : ''`)은 불량 있는 구분에만 실었다. 사용자가
         **두 안을 미리보기로 나란히 보고** 「7행 전부」를 택했다 — 「같은 기간이 여덟 번째」라는
         경고까지 본 선택이라 중복은 **의도된 것**이다. 되돌리지 말 것.
         ⚠ `hasDefect`는 남는다 — 아래 `isNote`(사항 칸이 자동 문구인가)가 여전히 쓴다.
           일자 칸의 조건이 아닐 뿐이다. */
      period,
      /* 🚨 2026-09-14 정정 — **일수도 함께 싣는다.** 종전엔 비웠다("기간을 여덟 번 말해도 일수까지
         여덟 번 말하지는 않는다"). 그런데 **갑지 엑셀 계획서 21칸(`planDays{row}`)은 일수를 찍고
         있어** 같은 회차의 두 산출물이 다른 표를 인쇄했다 — D-7이 깨진 자리였다. 사용자 확정: 찍는다.
         ⚠ `report9-actions`의 덧칠 경로(수기 보정만 있는 회차)도 **같이** 고쳐야 한다. 한쪽만
           고치면 자동 산출 회차와 수기 보정 회차가 서로 다른 표를 낸다. */
      days: d.actionPeriod ? String(d.actionPeriod.days) : '',
