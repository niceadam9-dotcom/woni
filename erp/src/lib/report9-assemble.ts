 ...
 *
 *  🎯 **사항 칸 = 「결과참조」 · 일자 칸 = 총 이행기간**(2026-09-11 사용자 지시).
 *    불량 내용은 본문에 나열하지 않고 첨부 점검표(별지 9호 8쪽)를 가리키기만 하며, 그 구분의
 *    이행기간으로 총 이행기간을 싣는다. **적용 대상은 「불량이 있는 구분」뿐**이다 —
 *    이상없음·해당없음 행은 종전대로 문구 + `—`(불량이 없으니 이행할 것도 없다).
 *
 *    경위(세 번 바뀐 자리다 — 되돌리기 전에 읽을 것):
 *    ① 설비 구분마다 `actionGroupPeriods`를 찍어 한 서식이 서로 다른 날짜를 말했다.
 *    ② 2026-09-09 총 이행기간으로 통일하니 **같은 기간이 여덟 번**(7행 + 필요기간) 인쇄됐다.
 *    ③ 2026-09-10 그래서 일자 칸을 통째로 「결과참조」로 바꿨다.
 *    ④ 2026-09-11 **사항·일자를 맞바꿨다.** ②의 여덟 번이 재발하지 않는 이유는 날짜가 붙는 행이
 *       **불량 있는 구분으로 한정**되기 때문이다(실무상 1~2행) — 7행 전체가 아니다.
 *
 *  ⚠ `rows`(사용자 입력 있음)와 `refer`(불량은 있는데 입력 없음)를 **한 낱말로 접는다**.
 *    둘의 차이는 「불량 내용을 적었는가」인데, 사항 칸이 어느 쪽이든 「결과참조」가 된 이상
 *    표에서 구별되지 않는다. 구별이 필요한 곳은 8쪽·현5이고 거기서는 그대로 갈라진다.
 *
...
export function annexPlanRows(d: Report9Data): AnnexPlanRow[] {
  ...
  // 총 이행기간 — 「이행조치 필요기간」 줄과 **같은 원천·같은 표기**(actionPlanPeriod → kdate).
  //   수기 보정(annex_inputs)만 있고 자동 산출이 없는 회차는 조립 뒤 report9-actions가 같은 값을 얹는다.
  const period = d.actionPeriod
    ? `${kdate(d.actionPeriod.startISO)} ~ ${kdate(d.actionPeriod.endISO)}`
    : ''
  return DEFECT_GROUPS.map(group => {
    const f = folds?.get(group)
    const mine = d.defectRows.filter(r => r.group === group)
    // 「이 구분에 불량이 있는가」 — 일자 칸의 유일한 조건이다.
    //   fold가 있으면 rows·refer가 그 뜻이고, 미공급이면 불량 행의 존재로 직접 판정한다
    //   (미공급을 '불량 없음'으로 단정하면 이행기간이 조용히 사라진다).
    const hasDefect = f ? (f.kind === 'rows' || f.kind === 'refer') : mine.length > 0
    return {
      group,
      // ⚠ 미공급(구 호출부·픽스처·대장 공란)이면 **자동 문구를 쓰지 않는다** — 종전처럼 그 구분의
      //   불량 내용을 그대로 싣는다. 종전 `?? []`는 미공급을 '전 구분 미해당'으로 읽어 7행을 전부
      //   「해당없음」으로 단정했다(2026-09-08 정정 — 8쪽·현5의 미공급 대조군과 축을 맞춘다).
      // ⚠ `hasDefect`로 쓰지 않는다 — 불리언은 `f.kind`를 좁히지 못해 마지막 갈래에서
      //   'rows'까지 색인으로 들어온다(DEFECT_FOLD_TEXT엔 그 키가 없다). 갈래에서 직접 좁힌다.
      content: !folds ? mine.map(r => r.content).join('\n')
        : !f ? ''
          : f.kind === 'rows' || f.kind === 'refer' ? DEFECT_FOLD_TEXT.refer
            : DEFECT_FOLD_TEXT[f.kind],
      /* 🚨 2026-09-20 (다섯 번째 판) — **이상없음·해당없음 행은 기간을 비운다**(사용자 지시
         「별지 10~11에서 이상없음과 해당없음은 이행기간이 조회되지 않도록」 — 엑셀 21칸도 동일).
         ④판(2026-09-11 「7행 전부」)을 사용자가 본인 결정으로 갱신한 것이다: 불량 없는 구분에
         기간이 서면 이행할 것이 없는 설비에 이행기간이 있는 것처럼 읽힌다.
         ⚠ 기간이 붙는 행 = 불량 있는 구분(rows·refer=「결과참조」 행)뿐. 미공급(folds 없음)은
           종전대로 7행 전부 — ok/na를 구별할 근거가 없어 좁히지 않는다(isNote와 같은 축).
         ⚠ 비운 행은 렌더(report1011.planTable)가 isNote 폴백으로 `—`를 찍는다(Q-5 b안 존속).
         ⚠ `report9-actions`의 덧칠 경로(수기 보정만 있는 회차)도 **같은 축**이다 — 한쪽만
           고치면 자동 산출 회차와 수기 보정 회차가 서로 다른 표를 낸다. */
      period: !folds || hasDefect ? period : '',
      /* 🚨 2026-09-14 정정 — 일수도 기간과 **한 벌**(갑지 엑셀 계획서 21칸과 같은 표). 2026-09-20부터
         기간과 같은 조건으로만 싣는다 — 기간 없는 행에 일수만 남으면 `(총 N 일)` 꼬리가 허공에 뜬다. */
      days: (!folds || hasDefect) && d.actionPeriod ? String(d.actionPeriod.days) : '',
      /* 사항 칸이 자동 문구(이상없음·해당없음)라는 표시 — **생산자가 표시한다**(렌더가 글자로
         알아보면 사용자가 조치 내용에 「해당없음」이라 적었을 때 오판한다).
         ⚠ 이제 이 값은 **일자 칸을 `—`로 만들지 않는다**. 기간이 **없을 때만** 렌더의 폴백으로
           쓰인다(report1011.planTable) — Q-5 b안(미대상 설비에 빈 자리표를 세우지 않는다)은
           그 구간에서 그대로 산다. */
      isNote: !!f && !hasDefect,
    }
  })
...
export async function assembleReport9(
  ...
  //   같은 판정을 보고서 탭 「전년도 업무 실시사항」 입력 화면(구 소방계획서 1.10)도 보여 줘야 해서,
  ...
  //    **별지 9호엔 그 칸이 없어**(서식 1.1 AR13이 받는다) 전기차만 켠 값이 여기서 「미반영」으로
  ...
  // ⚠ 안내가 가리키는 자리는 **보고서 탭 「전년도 업무 실시사항」**이다(51 3분리로 소방계획서 1.10에서
  //   이사). 44가 별지 9호 작성 패널 ③의 6칸을 걷어냈으므로 「작성 패널 ③에서 확정」은 이제 없는
  //   창구다 — F-8과 같은 죽은 안내(독립 판정 2026-09-08 적발). 낡은 자리를 가리키면 같은 부류다.
  ...
  if (!data.prevOpDone && !data.prevCompDone && !data.prevOpNone && !data.prevCompNone) {
    missing.push(`전년도(${prevYear}) 완료된 자체점검 이력 없음 — 2쪽 자체점검 칸 공란(보고서 탭 「전년도 업무 실시사항」에서 실시·미실시 확정 가능)`)
  }
  if (!data.eduDone && !data.eduNone) {
    missing.push(`전년도(${prevYear}) 소방안전교육 실적 없음 — 2쪽 교육훈련 칸 공란(서식 1.11.4 기록부 입력 또는 보고서 탭 「전년도 업무 실시사항」 확정)`)
  }
  if (!data.drillDone && !data.drillNone) {
    missing.push(`전년도(${prevYear}) 소방훈련 실적 없음 — 2쪽 교육훈련 칸 공란(서식 1.11.4 기록부 입력 또는 보고서 탭 「전년도 업무 실시사항」 확정)`)
  }
  return {
    data, missing,
