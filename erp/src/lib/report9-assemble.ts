...
export function annexPlanRows(d: Report9Data): AnnexPlanRow[] {
  ...
  return DEFECT_GROUPS.map(group => {
    ...
    return {
      ...
      period: hasDefect ? period : '',
      // ⚠ 비운다 — 총 일수는 「이행조치 필요기간」 줄이 단독으로 싣는다(`(총 N 일)`).
      //   여기에도 붙이면 한 서식이 같은 일수를 두 번 말한다.
      days: '',
      // 불량이 없는 줄(이상없음·해당없음)이면 일자 칸을 자리표 대신 `—`로(Q-5 b안).
      // **생산자가 표시한다** — 렌더가 글자로 알아보면 사용자가 조치 내용에 「해당없음」이라
      // 적었을 때 진짜 이행조치의 날짜가 조용히 사라진다.
      // 미공급(folds 없음)은 종전 렌더 그대로라 표시하지 않는다.
      isNote: !!f && !hasDefect,
    }
  })
}
