...
export type Form3AxisWarnings = {
  ...
  /** 미설치인데 그 항목을 겨눈 ○/× 응답이 있다 — 대장 누락 의심. 인쇄는 ／로 눌리지만(2026-09-03)
   *  응답 데이터는 남아 있어, 이 경고를 보고 1.4 대장에 체크하면 ○/×가 그대로 되살아난다. */
 ...
 *
 *  ── 미설치 = 무조건 ／ (2026-09-03 사용자 결정, image-51 강순건물) ─────────────
 *  1.4 대장 체크가 점검결과의 정본 축이다: **미체크 항목은 응답이 있어도 ／로 인쇄한다.**
 *  [ ] 체크 없는 행에 ○가 찍히는 모순(유도표지·피난유도선 실사고)을 막는다.
 *  종전(2026-08-21~09-02)에는 '대장 누락일 뿐 실점검일 수 있다'며 직접 겨눈 ○/×를 유지했으나
 *  지시로 번복됐다. 응답 데이터는 지우지 않으므로 — respondedNotInstalled 경고를 보고 대장에
 *  체크하면 ○/×가 그대로 되살아난다. 인쇄만 대장을 따른다.
 *
 *  응답의 귀속(어느 항목의 stat인가)은 종전 규칙 유지 — 경고 산출에 쓰인다:
 *    · 설치된 형제가 하나라도 있으면 → 응답은 그 형제 것(미설치로 번지지 않는다)
 *    · 형제가 전부 미설치면 → 종전대로 전개하되, 마크는 위 규칙이 ／로 누른다
...
export function rollUpForm3Results(
  ...
  for (const it of form3Items) {
    // 미체크(미설치)는 응답과 무관하게 ／ — 대장이 정본이다(2026-09-03, 위 주석). 응답은 statByItem에
    // 남아 respondedNotInstalled 경고가 되고, 대장에 체크하는 순간 아래 규칙으로 되살아난다.
    if (!installed.has(it)) { resultMarks[it] = 'N'; continue }
    const st = statByItem.get(it)
    // 순서가 규약이다: 불량 하나면 × → 양호 하나면 ○ → 응답은 있는데 전부 ／면 ／ → 무응답은 공란(키 없음).
 ...
 *    · 부모 행            → **항상 공란**(2026-09-03 사용자 결정, image-51 — 종전 '설치 하위가
 *                          없으면 부모 행에 롤업'을 번복). 서식 원문대로 부모 결과칸은 하위가
 *                          대신 말하는 자리라 어떤 마크도 찍지 않는다.
 ...
 *    (엑셀은 부모 칸이 원래 배선에 없다 — C6·Y6 verdict null. 이 규칙으로 PDF가 엑셀과 같아졌다) */
