...
export function PurchaseOrdersClient({ orders, items, partners }: {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="bg-paper border border-brand-line rounded-xl p-4 space-y-3">
          <div className="grid grid-cols-4 gap-3">
            <div className="space-y-1">
              <label className="text-xs text-ink-sub">거래처</label>
            ...
            <div className="space-y-1"><label className="text-xs text-ink-sub">발주일<span className="text-red-500 ml-0.5">*</span></label><DateInput value={form.order_date} onChange={e => setForm(p => ({ ...p, order_date: e.target.value }))} className={inputCls} /></div>
            <div className="space-y-1"><label className="text-xs text-ink-sub">입고예정일</label><DateInput value={form.expected_date} onChange={e => setForm(p => ({ ...p, expected_date: e.target.value }))} className={inputCls} /></div>
            <div className="space-y-1"><label className="text-xs text-ink-sub">메모</label><input value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} className={inputCls} /></div>
          </div>
          <div className="space-y-2">
            {lines.map((l, i) => (
              <div key={i} className="grid grid-cols-4 gap-2 items-center">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-ink-sub">{((parseFloat(l.quantity) || 0) * (parseFloat(l.unit_price) || 0)).toLocaleString()}원</span>
                  {lines.length > 1 && (
                    <button onClick={() => removeLine(i)} className="size-7 rounded hover:bg-red-50 flex items-center justify-center text-ink-faint hover:text-red-500"><Trash2 className="size-3.5" /></button>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="flex items-center gap-4">
            <button onClick={addLine} className="text-xs text-brand hover:underline flex items-center gap-1"><Plus className="size-3.5" />품목 추가</button>
            <span className="text-xs font-medium text-ink ml-auto">합계: {total.toLocaleString()}원</span>
          </div>
      ...
      <div className="space-y-2">
        {rows.length === 0 ? (
          <div className="py-12 text-center text-sm text-ink-sub bg-surface rounded-xl border border-line">발주 내역이 없습니다</div>
        ) : rows.map(po => (
          <div key={po.id} className="bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px] overflow-hidden">
            <div className="px-5 py-4 flex items-center gap-4">
              ...
              <span className="text-sm font-medium text-ink">{po.order_date}</span>
              <span className="text-xs text-ink-sub">{po.partner?.partner_name ?? '거래처없음'}</span>
              <span className="text-xs text-ink-faint">{po.purchase_order_lines.length}개 품목</span>
              <span className="text-sm font-semibold text-ink ml-auto">{po.total_amount.toLocaleString()}원</span>
              <select value={po.status} onChange={e => handleStatusChange(po.id, e.target.value)} disabled={isPending}
                className="h-7 rounded border border-line px-1 text-xs text-ink-sub outline-none">
                {Object.entries(STATUS_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
              <button onClick={() => setExpanded(p => p === po.id ? null : po.id)} className="text-ink-faint hover:text-brand transition-colors">
            ...
            {expanded === po.id && (
              <div className="border-t border-line px-5 py-3">
                <table className="w-full text-xs">
                  <thead><tr className="text-ink-sub">{['품목코드', '품목명', '수량', '단가', '소계'].map(h => <th key={h} className="py-1 text-left font-medium">{h}</th>)}</tr></thead>
                  <tbody>
                    {po.purchase_order_lines.map(l => (
                      <tr key={l.id} className="border-t border-line">
                        <td className="py-1.5 text-brand font-mono">{l.item?.item_code}</td>
                        <td className="py-1.5 text-ink">{l.item?.item_name}</td>
                        <td className="py-1.5 text-right text-ink-sub">{l.quantity} {l.item?.unit ?? ''}</td>
                        <td className="py-1.5 text-right text-ink-sub">{l.unit_price.toLocaleString()}</td>
                        <td className="py-1.5 text-right font-medium text-ink">{l.subtotal.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                {po.notes && <p className="text-xs text-ink-faint mt-2">{po.notes}</p>}
              </div>
            )}
          </div>
        ))}
