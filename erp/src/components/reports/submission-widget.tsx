...
export function SubmissionWidget({ myId, defaultMine }: { myId: string; defaultMine: boolean }) {
  ...
  return (
    <div id="submissions" className={`bg-white rounded-xl border border-[#c8c4d0] ${cardShadow} scroll-mt-4`}>
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-2 px-5 py-4 text-left">
        ...
        <TableProperties className="size-4 text-[#7b68ee]" />
        <h2 className="text-sm font-semibold text-[#090c1d]">제출 현황</h2>
        <span className="text-[10px] text-[#b0acd6]">최근 90일 · 9호·배치확인서·10·11호</span>
        {/* 요약 스트립 — 펼치지 않아도 위험 신호가 보이게 */}
        <span className="ml-auto flex items-center gap-1.5 text-[11px] shrink-0">
          {!board && !err && <Loader2 className="size-3.5 animate-spin text-[#b0acd6]" />}
          ...
          {s && (
            <>
              ...
              <span className={`px-2 py-0.5 rounded-full ${s.r9NotSubmitted > 0 ? 'bg-amber-50 text-amber-700' : 'bg-[#eeecf8] text-[#514b81]'}`}>
                9호 미제출 {s.r9NotSubmitted}
              </span>
              <span className={`px-2 py-0.5 rounded-full ${s.certMissing > 0 ? 'bg-amber-50 text-amber-700' : 'bg-[#eeecf8] text-[#514b81]'}`}>
      ...
      {open && (
        <div className="border-t border-[#f0eefa] px-5 pb-4 pt-3">
          {board
            ...
            : <p className="text-xs text-[#b0acd6] py-4 inline-flex items-center gap-1"><Loader2 className="size-3 animate-spin" /> 불러오는 중…</p>}
        </div>
      )}
    </div>
  )
