...
export function InspectionReport9Client({
  ...
  return (
    <div className="bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px] p-5">
      ...
      {variant === 'exterior' && (
        <p className="text-form-xs text-ink-meta -mt-1 mb-3">
      ...
      {canManage && (
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {checks.some(c => !c.ok) && (
            <span className="text-form-xs text-amber-600">미비 항목은 빈 칸으로 출력됩니다 (fail-soft)</span>
          )}
        </div>
      )}
      ...
      {canManage && variant !== 'exterior' && defectsInfo.total > 0 && (
        <div className="mt-3 pt-3 border-t border-dashed border-brand-line-soft space-y-1.5">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="ml-auto inline-flex items-center gap-1.5">
              <button onClick={() => setCompose('report10')} disabled={isPending || busy}
                ...
                className="inline-flex items-center gap-1 h-7 px-2.5 rounded-lg border border-brand-line text-form-xs text-brand hover:bg-brand-tint disabled:opacity-50">
          ...
          <p className="text-form-2xs text-ink-meta">계획·완료 입력은 아래 불량내역의 [이행계획·조치 완료]에서 — 10호는 9호의 법정 첨부, 11호 첨부(전후 사진·계약서)는 불량내역 사진 슬롯 활용</p>
        </div>
      )}
      ...
      {job?.status === 'done' && job.missing && job.missing.length > 0 && (
        <p className="text-form-xs text-amber-600 mt-2">누락: {job.missing.join(' · ')}</p>
      )}
