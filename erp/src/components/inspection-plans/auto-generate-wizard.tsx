  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-ink-sub mt-1">전월 계획 기반으로 이달 점검계획 초안을 자동 생성합니다.</p>
      </div>
      ...
      <div className="flex items-center gap-0">
        {STEPS.map((label, idx) => (
          <div key={idx} className="flex items-center">
            ...
            {idx < STEPS.length - 1 && <ChevronRight className="size-4 text-ink-faint mx-1" />}
          </div>
        ))}
      </div>
      ...
      {/* Step 0: 조건 설정 */}
      {step === 0 && (
        <div className="bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px] p-6 space-y-5">
          {prevPlan ? (
            <div className="flex items-start gap-3 p-3 bg-brand-tint rounded-lg">
              <Calendar className="size-4 text-brand mt-0.5 shrink-0" />
              <div>
                <p className="text-sm font-medium text-ink">참조 계획 발견</p>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-medium text-ink">점검 대상 고객 선택</p>
              <div className="flex gap-2 text-xs text-ink-sub">
                <button onClick={() => setSelectedCustomers(customers.map(c => c.id))} className="hover:text-brand">전체선택</button>
                <span>/</span>
                <button onClick={() => setSelectedCustomers([])} className="hover:text-brand">전체해제</button>
              </div>
            </div>
            <div className="max-h-60 overflow-y-auto space-y-1 border border-brand-line-soft rounded-lg p-2">
              {customers.map(c => (
                <label key={c.id} className="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer hover:bg-paper">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-sm text-ink flex-1">{c.customer_name}</span>
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                    c.inspection_type === '종합' ? 'bg-brand-tint text-brand' : 'bg-blue-50 text-blue-600'
          ...
          <div className="flex justify-end">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-brand text-white text-sm font-medium rounded-lg hover:bg-brand-strong transition-colors disabled:opacity-50"
            >
      ...
      {step === 1 && (
        <div className="bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px] overflow-hidden">
          <div className="px-5 py-4 border-b border-brand-line-soft flex items-center justify-between">
            <p className="text-sm font-semibold text-ink">초안 항목 ({draftItems.length}건) — 날짜·담당자 수정 가능</p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-paper border-b border-brand-line-soft">
                  {['건물명','유형','차수','점검예정일','담당직원'].map(h => (
                    <th key={h} className="text-left text-xs font-medium text-ink-sub px-4 py-3">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={idx} className="border-b border-paper last:border-0">
                    ...
                    <td className="px-4 py-2">
                      <DateInput
                        ...
                        className="text-xs border border-line rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-brand"
                      />
                    </td>
                    <td className="px-4 py-2">
                      <select
          ...
          <div className="px-5 py-4 border-t border-brand-line-soft flex justify-between">
            <button onClick={() => setStep(0)} className="flex items-center gap-1.5 text-sm text-ink-sub hover:text-brand transition-colors">
      ...
      {/* Step 2: 확정 */}
      {step === 2 && (
        <div className="bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px] p-8 text-center space-y-4">
          ...
          <div className="flex gap-3 justify-center pt-2">
            <button
              ...
              className="px-5 py-2.5 text-sm border border-line rounded-lg text-ink-sub hover:bg-paper transition-colors"
            >
              나중에 확정
            </button>
            <button
