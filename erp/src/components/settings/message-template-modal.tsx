        ...
        <div className="fixed inset-0 z-50 bg-black/30 flex items-center justify-center p-4" onClick={() => setOpen(false)}>
          <div className="bg-surface rounded-xl border border-line shadow-lg w-full max-w-4xl max-h-[88vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-2 px-5 py-3 border-b border-brand-line-soft sticky top-0 bg-surface">
              ...
              <h2 className="text-sm font-semibold text-ink">{label} 문구</h2>
              <button onClick={() => setOpen(false)} className="ml-auto text-ink-faint hover:text-ink"><X className="size-4" /></button>
            </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="rounded-lg border border-brand-line-soft bg-paper p-3 space-y-2">
                        {data.template.subject !== null && (
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    <button onClick={() => setOpen(false)}
                      className="h-8 px-3 rounded-lg border border-brand-line text-xs text-ink-sub hover:bg-paper">닫기</button>
                    {data.canEdit && (
                      <button onClick={save} disabled={isPending || !data.storageReady}
                        className="inline-flex items-center gap-1 h-8 px-3 rounded-lg bg-brand hover:bg-brand-strong text-white text-xs font-medium disabled:opacity-50">
