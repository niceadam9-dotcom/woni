...
export function RecommendAssignClient({ customerId, employeeId, employeeName, regionLabel }: {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-form-sm text-ink-sub">
      이 지역({regionLabel}) 담당: <span className="font-medium text-ink">{employeeName}</span>
      <button onClick={assign} disabled={isPending}
        className="inline-flex items-center gap-1 h-6 px-2 rounded-lg bg-brand hover:bg-brand-strong text-white text-form-xs font-medium disabled:opacity-50">
