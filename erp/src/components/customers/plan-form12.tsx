    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-brand-line-soft bg-brand-tint p-4">
        <div className="flex items-center gap-2 mb-2">
          <p className="text-form-sm font-semibold text-ink-sub">1.2.1 구역별 세부현황</p>
          {canManage && (
            <button onClick={autoFloors} className="ml-auto inline-flex items-center gap-1 h-form-7 px-2 rounded-lg border border-brand-line text-form-xs text-brand hover:bg-brand-tint">
      ...
      <div className="rounded-xl border border-brand-line-soft bg-brand-tint p-4">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          <p className="text-form-sm font-semibold text-ink-sub">1.2.2 화재취약장소</p>
          {canManage && HAZARD_PRESETS.map(pz => (
            <button key={pz.place}
              onClick={() => { setHazards(p => [...p, { ...pz, risks: [...pz.risks] }]); setDirty(true) }}
              className="h-form-6 px-2 rounded-full border border-brand-line text-form-xs text-brand hover:bg-brand-tint">
              + {pz.place}
            </button>
          ))}
        </div>
          ...
          {hazards.map((h, i) => (
            <div key={i} className="flex items-center gap-1.5 flex-wrap">
              <input value={h.place} onChange={e => setHazard(i, { place: e.target.value })} disabled={!canManage}
                placeholder="장소" className="h-form-7 w-28 rounded border border-brand-line bg-surface px-1.5 text-form-sm outline-none focus:border-brand" />
              <input value={h.loc} onChange={e => setHazard(i, { loc: e.target.value })} disabled={!canManage}
                placeholder="위치 (예: 지하 1층)" className="h-form-7 w-32 rounded border border-brand-line bg-surface px-1.5 text-form-sm outline-none focus:border-brand" />
              {RISKS.map(r => (
