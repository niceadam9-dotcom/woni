    ...
    <div className="space-y-4">
      ...
      <div id="c-3.4" className="scroll-mt-4 rounded-xl border border-brand-line-soft bg-brand-tint p-4 space-y-2">
            ...
            {/* 소방계획서_11 §13-A — 피난안내도는 [지도·사진] 슬롯(evac_*)이 단일 원천.
                슬롯 UI는 2026-08-08부터 서식 1.3 안에 있다(전용 트리 노드 폐지).
                ...
                <p className="mt-1.5 text-form-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2 py-1">
                  ...
                  {canManage && (
                    <button type="button" className="ml-1 underline hover:text-red-600"
                      onClick={() => {
                        if (!window.confirm('서식에 저장된 옛 피난경로도를 삭제할까요? (1.3 [지도·사진]의 이미지는 유지됩니다)')) return
      ...
      {/* 3.6 유형별 방법 */}
      <div id="c-3.6" className="scroll-mt-4 rounded-xl border border-brand-line-soft bg-brand-tint p-4 space-y-2">
