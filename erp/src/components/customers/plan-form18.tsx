...
export function PlanForm18({ data }: { data: Form18Data }) {
  const row = (label: string, value: string | null | undefined) => (
    <div className="flex items-baseline gap-2 py-1 border-b border-brand-line-soft last:border-0">
      <span className="w-32 shrink-0 text-[11px] text-ink-soft">{label}</span>
      <span className="text-xs text-ink">{value?.trim() ? value : <span className="text-ink-faint">미입력</span>}</span>
    </div>
  )
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-brand-line-soft bg-brand-tint p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <p className="text-xs font-semibold text-ink-sub">1.8 소방안전관리 업무대행 현황</p>
          <span className="text-[11px] text-ink-faint ml-auto">자동 표시 — 수정은 회사 정보·고객 기본정보에서</span>
        </div>
        <p className="text-[11px] font-medium text-brand mb-1">대행업체 (회사 정보)</p>
        ...
        <p className="text-[11px] font-medium text-brand mt-3 mb-1">대행 계약·대상물</p>
      ...
      <p className="text-[11px] text-ink-faint">※ 업무대행 범위·기간 등 상세 계약 조건은 계약 문서를 따릅니다 — 본 서식은 현황 표시용.</p>
    </div>
  )
}
