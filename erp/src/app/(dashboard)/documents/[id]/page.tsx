...
import { RecallButton } from '@/components/documents/recall-button'
...
const STATUS_MAP: Record<string, { label: string; className: string }> = {
  ...
  pending:  { label: '결재중',   className: 'bg-blue-50 text-blue-600' },
...
export default async function DocumentDetailPage({ params }: Props) {
  ...
  // 열람 권한: 기안자, 결재자, 관리자만
  const isApprover = doc.author_id !== profile.id
  if (isApprover && profile.role !== 'admin' && profile.role !== 'manager') {
  ...
  const { data: approverProfilesRaw } = approverIds.length > 0
    ? await admin.from('profiles').select('id, name, email, position').in('id', approverIds)
    : { data: [] }
  ...
  const statusInfo = STATUS_MAP[doc.status] ?? { label: doc.status, className: '' }
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {isAuthor && doc.status === 'pending' && <RecallButton documentId={doc.id} />}
