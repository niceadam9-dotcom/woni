 ...
 *
 *  소방계획서_21 R8-3: 종전 /inspection-ledger 단독 메뉴였으나 customers를 통째로 읽는 뷰라
 *  고객 관리의 탭으로 흡수했다(구 경로는 리다이렉트). 열 구성이 고객 목록과 달라
 *  화면은 기존 LedgerClient를 그대로 쓴다. */
export default async function CustomerLedgerPage() {
  ...
  return (
    <div className="space-y-4">
      <CustomerViewTabs />
      <LedgerClient rows={rows} canViewFee={canViewFee} />
    </div>
  )
}
