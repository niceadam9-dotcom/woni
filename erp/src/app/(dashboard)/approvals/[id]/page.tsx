  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <div className="flex items-start gap-3">
        <Link href="/approvals" className="text-ink-sub hover:text-brand transition-colors mt-1 shrink-0">
          <ArrowLeft className="size-5" />
        </Link>
        <div className="min-w-0">
          <span className="text-xs text-ink-sub">{TEMPLATE_MAP[doc.template_type] ?? doc.template_type}</span>
          <h1 className="text-xl font-bold text-ink mt-0.5 break-words">{doc.title}</h1>
        </div>
      </div>
      ...
      {isMyTurn && doc.status === 'pending' && (
        <div className="bg-surface rounded-xl border border-brand-tint p-5 space-y-3">
          <h2 className="text-sm font-semibold text-ink">결재 처리</h2>
