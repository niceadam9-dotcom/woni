    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-ink">회의록</h1>
            <p className="text-sm text-ink-sub mt-0.5">회의 결과·결정사항을 기록합니다</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/board" className="h-9 px-3 rounded-lg border border-line text-sm text-ink-sub hover:bg-paper transition-colors flex items-center">
            게시판
          </Link>
      ...
      <form method="GET" action="/board/meeting-notes" className="flex flex-wrap items-center gap-2">
        ...
        {q && <a href="/board/meeting-notes" className="h-9 px-3 rounded-lg border border-line text-sm text-ink-sub hover:bg-paper transition-colors flex items-center">초기화</a>}
      ...
      <div className="bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px,rgba(18,43,165,0.08)_0px_6px_6px_-3px,rgba(18,43,165,0.08)_0px_12px_12px_-6px] overflow-hidden">
        {rows.length === 0 ? (
          <div className="py-16 text-center text-sm text-ink-sub">등록된 회의록이 없습니다</div>
        ) : (
            ...
            <tbody className="divide-y divide-line">
              {rows.map(r => (
                <tr key={r.id} className="hover:bg-paper transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-ink-strong">{r.meeting_date}</td>
                  <td className="px-4 py-3 text-xs text-ink-sub">{r.location ?? '-'}</td>
                  <td className="px-4 py-3 text-xs text-ink-sub max-w-[150px] truncate">{r.participants ?? '-'}</td>
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/board/meeting-notes/${r.id}`} className="text-xs text-brand hover:underline font-medium">상세보기</Link>
                  </td>
                </tr>
              ))}
            </tbody>
