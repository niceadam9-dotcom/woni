  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-1.5 text-sm text-ink-sub">
        <Link href="/board" className="hover:text-brand flex items-center gap-1"><LayoutList className="size-3.5" />게시판</Link>
        <ChevronRight className="size-3.5 text-ink-faint" />
        <Link href={`/board/${id}`} className="hover:text-brand">게시물</Link>
        <ChevronRight className="size-3.5 text-ink-faint" />
        <span className="text-ink font-medium">수정</span>
      </div>
      <h1 className="text-xl font-bold text-ink">게시물 수정</h1>
      <PostFormClient
