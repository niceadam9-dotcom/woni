  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-1.5 text-sm text-ink-sub">
        ...
        <span className="text-ink font-medium truncate max-w-[200px]">{p.title}</span>
      </div>

      <div className="max-w-3xl bg-surface rounded-xl border border-line shadow-[rgba(18,43,165,0.08)_0px_1px_1px_-0.5px,rgba(18,43,165,0.08)_0px_3px_3px_-1.5px] overflow-hidden">
        {/* 헤더 */}
        <div className="px-6 py-5 border-b border-line">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                {p.category && <span className="text-xs px-2 py-0.5 rounded-full bg-brand-tint text-brand">{p.category.name}</span>}
                {p.is_notice && (
              ...
              <div className="flex items-center gap-3 mt-2 text-xs text-ink-sub">
        ...
        <div className="px-6 py-6">
          <div className="text-sm text-ink whitespace-pre-wrap leading-relaxed">{p.content}</div>
        </div>
      </div>

      <div className="max-w-3xl">
        <Link href="/board" className="text-sm text-brand hover:underline">← 목록으로</Link>
      </div>
    </div>
  )
}
