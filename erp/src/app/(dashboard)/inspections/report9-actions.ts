...
async function assembleAnnex1011(
  ...
  if (kind === 'report10') {
    ...
    //     두 기간을 말한다.
    //   🚨 2026-09-20 (다섯 번째 판) — **자동 문구 줄(isNote=이상없음·해당없음)은 덧칠하지 않는다.**
    //     ④판(2026-09-11 「7행 전부」)을 사용자가 본인 결정으로 갱신했다: 「별지 10~11에서
    //     이상없음과 해당없음은 이행기간이 조회되지 않도록」. 조립본(annexPlanRows)·엑셀 21칸과
    //     **같은 축**이어야 한다 — 한쪽만 고치면 자동 산출이 있는 회차와 수기 보정만 있는
    //     회차가 서로 다른 표를 인쇄한다.
    //   ⚠ Q-5 b안은 그대로 산다: isNote 줄은 기간이 없으므로 렌더가 `—`를 찍는다.
    //   🚨 2026-09-14 정정 — **일수도 함께 찍는다.** 종전엔 `days: ''`로 비웠다("기간을 여덟 번
    //     말해도 일수까지 여덟 번 말하지는 않는다"). 그 결과 **갑지 엑셀 계획서 21칸은 일수를
    //     찍는데 PDF 7행만 안 찍어** 두 산출물이 달랐다. 기간이 붙는 행에는 일수도 한 벌이다.
    if (data.planRows && data.totalPeriod) {
      const total = data.totalPeriod
      const days = data.totalDays ?? ''
      data.planRows = data.planRows.map(r => (r.isNote ? r : { ...r, period: total, days }))
    }
  } else {
