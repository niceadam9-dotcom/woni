...
export async function GET(req: NextRequest) {
  ...
  // 기준일은 생성기가 결정(점검계획일 → 최초 점검시작일 → 사용승인일) — 모두 없으면 0건
  const { data: customers, error: custErr } = await admin
    .from('customers')
    .select('id, customer_name, inspection_type, use_approval_date, plan_anchor_date, assigned_employee_id')
    .eq('is_active', true)
  ...
  type CustRow = {
    ...
    use_approval_date: string | null; plan_anchor_date: string | null; assigned_employee_id: string | null
  }
