...
console.log('\n[14] 3.3 피난인원현황 — 1.2.1과 같은 값인가')
{
  ...
  if (vc.ok) {
    ...
    // 2026-09-18: 1.2.1 예산이 8(토큰 수)에서 14(A22가 끊는 실제 표)로 늘었다 — 두 시트의
    //   예산이 여전히 다르다는 전제는 그대로다(19 ≠ 14). 겹치는 14행을 맞댄다.
    check('두 시트 예산이 다르다는 전제(3.3 19행 ≠ 1.2.1 14행)', ZONE_ROWS === 14 && EVAC3_ROWS > ZONE_ROWS,
      `1.2.1 ${ZONE_ROWS}행 · 3.3 ${EVAC3_ROWS}행`)
    ...
    // 2026-09-18: 1.2.1 예산 8→14로 대조 범위가 40→70칸이 됐다(예산에서 파생되므로 자동)
    check('대조가 실제로 돌았다(0건이면 공허)', cmp === 70, `${cmp}칸`)
