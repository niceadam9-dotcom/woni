...
// 🚨 **분모가 줄 수도 있다** — 2026-09-18 1.11.4 뒷쪽 사진 4칸(A11·AB11·A13·AB13)에
...
// ⭐ 2026-09-21 「사진 표지」로 표지 정보 블록 2칸(소재지 M7·작성 M8)이 신설돼 1,806→1,808.
//   분모가 **늘어난** 경우다(위 감소와 반대 방향) — 값이 들어갈 자리가 실제로 둘 생겼다.
check('값 슬롯 1,808칸', slots === 1808, `${slots}칸`)
check('상자 658칸', boxes === 658, `${boxes}칸`)
check('배선된 값 슬롯 ≥ 962', wired >= 962, `${wired}칸 (${(wired / slots * 100).toFixed(1)}%)`)
...
const EXPECT: Array<[string, string, string]> = [
  ...
  //   🚨 표본은 **미배선 칸**이어야 산다 — 2026-09-18 1.7.1!A10을 배선하자 그 칸이 빈칸
  //     목록에서 빠져 이 단언이 붉어졌다(표본 소진). 다른 열로 갈아끼운다: 한 시트에
  //     몰리지 않게 열이 다른 둘을 고른다(A열·AA열 — 둘째 패스가 열을 타는지 본다).
  ['1.2.1 구역별 세부현황', 'A10', '동'],
  ['1.2.1 구역별 세부현황', 'AA10', '야간'],
...
const fixture = {
  ...
  forms: {
    ...
    evacDetail: [{ facility: MARK('EVDET'), location: '2층', status: '양호' }],
 ...
 *  여기서 빨강이 된다(핀 FIRE_PLAN_SAMPLE_CELLS와 짝). */
{
  ...
  /* ⭐ 1.15 가스안전공사 번호 항등 (2026-09-18) — 엑셀 값(fire115_gas_tel)이 PDF
   *  「비상연락처」에 실제로 인쇄되는 그 번호여야 한다(리터럴 둘이 갈라지면 여기서 빨강). */
  const { buildFirePlanValues } = await import('../src/lib/fire-plan-xlsx-values.ts')
  const gasTel = String(buildFirePlanValues(fixture).get('fire115_gas_tel') ?? '')
  check('1.15 가스안전공사 번호 = PDF 비상연락처', gasTel.length > 0 && html.includes(gasTel), gasTel)
}
