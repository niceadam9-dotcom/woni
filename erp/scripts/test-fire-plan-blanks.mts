...
check('배선된 값 슬롯 ≥ 866', wired >= 866, `${wired}칸 (${(wired / slots * 100).toFixed(1)}%)`)
check('배선된 상자 ≥ 369', wiredBoxes >= 369, `${wiredBoxes}칸 (${(wiredBoxes / boxes * 100).toFixed(1)}%)`)
...
const RED_EXPECTED = [
  '1.11.2 소방훈련·교육 세부계획', '1.11.3 소방훈련 시나리오',
  '1.14.2 화재예방 및 홍보 결과',
