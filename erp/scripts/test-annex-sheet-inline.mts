...
try {
  ...
  await page.waitForSelector('button:has-text("이 설비 저장")', { timeout: 20000 })
  ...
  // ── 4) 저장 → DB 반영 + 새로고침 없이 요약 갱신 ──
  await page.locator('button:text-is("○")').first().click()
  await page.click('button:has-text("이 설비 저장")')
  await page.waitForSelector('text=항목 저장됨', { timeout: 20000 })
  ...
  await page.locator(`button:has-text("${INSTALLED}")`).first().waitFor({ timeout: 20000 })
  ...
  check('비담당 — 저장 버튼 미노출', (await page.locator('button:has-text("이 설비 저장")').count()) === 0)
  const { data: after } = await raw.from('inspection_sheet_responses').select('item_code').eq('inspection_id', inspId)
  check('비담당 — DB 무변화(2건 유지)', (after ?? []).length === 2, String((after ?? []).length))
} catch (e) {
