// 불량 0건 회차의 ⑤⑥ **즉시 감춤 + ④ 기본 표시** (2026-09-20 사용자 지시).
//
//   불량(✕ ∪ 불량내역)이 0이면 작업대·달력·목록·사이드바·모바일이 **같은 표시 축**(visibleStepNums)으로
//   ⑤⑥을 그리지 않는다. ④(별지 9호 소방서 제출)는 법정 의무라 **항상 그린다** — 점검표를 아직
//   안 채운 새 회차도 처음부터 ①②③④ 4단계다.
//
// 🚨 이 파일은 **계약이 두 번 뒤집힌 자리**다. 2026-09-10 판(6 → 4 → 3)은 두 게이트를 전제했고,
//    48차수(09-11 「즉시 감춤으로 통일」)가 그 둘을 폐지하며 ④까지 감췄는데, 2026-09-20 사용자
//    지시 「④ 소방서 제출은 기본 포함」이 ④만 도로 살렸다. 종전 단언(「①②③ 3단계」·「④ 감춰짐」)은
...
//    4 → 6 → 4를 차례로 단언한다(음성·양성·복귀). 한쪽만 물으면 「⑤⑥을 늘 감춘다」도, 「한 번도 안
//    감춘다」도 초록으로 빠져나간다.
//
// 🎯 마지막 블록이 여전히 가장 비싼 단언이다 — **화면이 줄어도 의무는 안 줄어든다**.
//    별지 9호는 불량 유무와 무관한 법정 의무라(시행규칙 제23조제2항), ①②③이 다 차도
...
try {
  ...
  // ── ① 새 회차(점검표 전) — 처음부터 ①②③④ 4단계 ─────────────────────────────────
  // 🚨 종전 계약(48차수)은 여기서 「3단계·④ 감춰짐」이었다. 2026-09-20 사용자 지시로 ④는
  //    기본 포함이다 — 불량이 생기면 ②에서 6으로 **되돌아오는지**가 안전망이다.
  await open()
  {
    const n = await barCount()
    check('새 회차 — 처음부터 ①②③④ 4단계(⑤⑥만 즉시 감춤)', n === 4, `${n}단계`)
    check('새 회차 — ④ 소방서 제출이 기본 표시(2026-09-20)', await hasStep('submit9'))
    check('새 회차 — ⑤ 감춰짐', !(await hasStep('repair')))
    check('새 회차 — ⑥ 감춰짐', !(await hasStep('submit11')))
    // 분모도 보이는 것으로 센다 — 4개가 보이는데 6/6이라 말하면 화면과 숫자가 어긋난다
    check('새 회차 — 진행률 분모가 4', await page.isVisible('text=/\\d\\/4 단계 완료/'))
    // 종전 회색 「해당없음」 행의 문구가 남으면 행만 지운 반쪽이다
    check('새 회차 — 「불량이 생기면 활성화」 안내가 없다',
      !(await page.isVisible('text=불량이 생기면 활성화')))
    // 문서로 가는 길도 그대로 남는다 — 합격 회차야말로 별지 9호를 제출해야 한다.
    // 라벨 「별지서식」→「보고서」 개명(2026-09-20 탭 재편) — 링크 요소로 좁혀 잡는다
    check('새 회차 — [회차 문서] 창구는 남는다(구 별지서식→보고서→회차 문서)',
      await page.isVisible('a[href*="tab=annex"]:has-text("회차 문서")'))
  }
  ...
  // 한 방향만 보면 「한 번 6이 되면 영영 6」이 초록으로 빠져나간다(표시 축은 되돌아와야 한다).
  {
    ...
    await open()
    const n = await barCount()
    check('정정 후 — 다시 4단계(표시 축이 되돌아온다)', n === 4, `${n}단계`)
    check('정정 후 — ⑤ 다시 감춰짐', !(await hasStep('repair')))
    check('정정 후 — ④는 그대로 남는다(감춤 대상이 아니다)', await hasStep('submit9'))
  }

  // ── ④ 🎯 의무 축 무손상 — ①②③이 다 차도 DB는 completed가 아니다 ─────────────────
  // ①은 응답으로, ②③은 사유 완료 마커로 채운다(제품 경로 `forceCompleteStepAction`과 같은 형태).
  // ④가 화면에 돌아왔으므로(2026-09-20) 이제 화면도 3/4를 그린다 — 화면과 의무가 같은 말을 한다.
  // 완료 판정의 분모는 여전히 activeStepNums(=①②③④)이고, 별지 9호 미제출인 이 회차는
  // completed가 되면 안 된다. 여기가 붉어지면 법정 보고가 조용히 건너뛰어진 것이다.
  {
    const { error: mErr } = await raw.from('activity_logs').insert([2, 3].map(n => ({
      ...
      metadata: { step_num: n, reason: 'E2E 의무 축 대조 — ④ 이외의 단계를 전부 채운다' },
    })) as never)
    check('양성 표본 — ②③ 사유 완료 마커 저장 성공(아래 단언의 전제)', !mErr, mErr?.message ?? '')

    await open()           // 상세 진입이 syncInspectionSteps를 발화시킨다
    const n = await barCount()
    check('의무 축 — 보이는 단계는 여전히 4개', n === 4, `${n}단계`)
    check('의무 축 — 화면은 3/4 단계 완료를 그린다(④만 미완 — 화면과 의무가 일치)',
      await page.isVisible('text=/3\\/4 단계 완료/'))

    const st = await statusOf()
    check('🎯 의무 축 — ①②③이 다 차도 inspections.status는 completed가 아니다 (별지 9호 미제출)',
