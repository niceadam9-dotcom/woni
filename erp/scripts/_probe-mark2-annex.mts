// 🚫 폐기(2026-09-07, 소방계획서_44) — **이 프로브는 이제 실패하는 것이 정상이다. 회귀가 아니다.**
//    검사 대상이던 6칸(자체점검·교육훈련·소방계획서 작성/보관)이 별지 9호 패널에서
//    소방계획서 서식 1.10 「전년도 업무 실시사항」으로 이관됐다(annex-fields.tsx 주석 참조).
//    같은 왕복을 새 자리에서 재는 것은 scripts/_probe-44-prev-year-duty.mts 다.
//
// 이하 폐기 전 원문 ─────────────────────────────────────────────────────────
