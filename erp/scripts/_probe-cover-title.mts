...
try {
  ...
  /* ①ⓓ 가장 짧은 이름은 **원본 32pt보다 확실히 크다**.
     ⚠ 2026-09-21 갈아끼움 — 종전 이 자리는 「두 배 이상(≥64pt)」이었다. 같은 날 사용자가
       **제목을 절제하고 사진을 주역으로** 하기로 다시 정해 상한이 96pt→54pt가 됐고
       (`475b25d6`), 짧은 이름은 갈라지지 않고 한 줄로 간다. 구계약을 지우지 않고 **새 상한에
       맞춰 반대 방향으로 바꾼다** — 여기가 지키는 뜻(「원본보다는 확실히 크다」)은 그대로다.
     ⚠ 상한·줄 나눔 **규칙 자체**는 `test-cover-title-size.mts`(순수 단언)가 정본이다.
       여기는 **산출물에서 그 결과가 실제로 나오는가**만 묻는다 — 계약을 두 벌 두지 않는다. */
  check('① 가장 짧은 이름은 원본 32pt보다 크고 새 상한 54pt 안이다',
    byEm[0].sz > 32 && byEm[0].sz <= 54, `${byEm[0].name}=${byEm[0].sz}pt`)
} catch (e) {
