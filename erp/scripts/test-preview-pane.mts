...
try {
  ...
  //    9호 고유값 칸이 전부 살아 있고, 접힌 2열 안에서 실제로 닿을 수 있는지(스크롤로 보이게
  //    되는지)를 같은 자리에서 함께 고정한다.
  //    2026-09-07(소방계획서_44): 분모가 8 → 2로 줄었다. **높이를 벌려고 지운 것이 아니라**
  //    2쪽 3행 6칸이 별지 9호 고유값이 아니어서 소방계획서 1.10으로 이관한 것이다. 그 6칸이
  //    여기 되살아나면(=확정 창구 이원화) 이 단언이 다시 붉어져야 한다 — 그래서 정확한 수로 못박는다.
  ...
  if (f9) {
    // annex-fields.tsx FIELD_DEFS.report9 = 2칸 (보고일·비고) — 나머지 6칸은 소방계획서 1.10으로 이관(44)
    check(`④ 9호 고유값 2칸이 전부 살아 있다 (실측 ${f9.n}칸)`, f9.n === 2, f9.labels.join('/'))
