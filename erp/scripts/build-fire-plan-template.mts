...
const COL_EDGE_NUDGES: ColEdgeNudge[] = [
  ...
  /* ── 2026-09-16 사용자 지적 image-2 「한줄로 만들어줘」 ──────────────────────────
   * 같은 자(`measureLines`)로 전 워크북 상자 칸 658개를 다시 재니 **접힌 칸이 정확히 셋**이고
   * 사용자가 든 것은 그 첫째다. 셋 다 같은 부류라 함께 편다(하나만 고치면 나머지 둘이
   * 「고쳐진 형제」 옆에서 더 도드라진다).
   *
   * 🚨 어느 쪽 경계를 미느냐가 **결과를 가른다** — 반대쪽을 밀면 값 칸이 좁아진다.
   *   1.1에서 오른쪽(edge 9)을 미는 안을 버린 이유: 그 경계 오른쪽은 `{{use_approval_date}}`
   *   같은 **값 칸**이라 8→7칸이 되면 날짜가 접힌다. 왼쪽(edge 7)을 당기면 줄어드는 것은
   *   여유가 큰 「 명」·「☐ 기계식」·「☐ 휴일」뿐이다. */
  { table: 5, edge: 7, to: 43, why: '1.1 인원현황 「☐최대수용인원」 8→9칸' },
  /* 1.9 근무형태 — 왼쪽(edge 13)은 못 당긴다. 이미 이웃 경계(map[12]=35)에 붙어 있어
   * 단조 증가가 깨진다(가드가 선다). 그래서 오른쪽을 민다 — 줄어드는 「☐ 교대직 (  조  교대)」는
   * 18칸이라 한 칸 내줄 여유가 크다. */
  { table: 24, edge: 17, to: 43, why: '1.9 근무형태 「☐ 상근직」 6→7칸' },
  /* 3.5 시설이용자 — 여기만 **두 칸**이 모자란다(5칸 65px에 「☐ 장애인」 58px + 들여쓰기 21px).
   * 6칸으로는 1px 차이로 여전히 접힌다 — 한 칸씩 미는 버릇으로 고치면 헛수고가 된다. */
  { table: 90, edge: 14, to: 46, why: '3.5 시설이용자 「☐ 장애인」 5→7칸' },
]
...
const ONE_LINE_CELLS: Record<string, string> = {
  ...
  // ③ 1.1 인원현황 — 넓힌 쪽 하나에 **좁아진 쪽 셋**. 좌표는 조정 뒤라 AS19가 AR19로 밀렸다
  '1.1 건축물 일반현황!AR19': '인원현황 ☐최대수용인원 (넓힌 쪽 8→9칸)',
  '1.1 건축물 일반현황!AJ19': '좁아진 쪽 — 인원현황 「 명」(여유가 가장 큰 기증자)',
  '1.1 건축물 일반현황!AJ14': '좁아진 쪽 — 주차장 ☐ 기계식',
  '1.1 건축물 일반현황!AJ17': '좁아진 쪽 — 운영시간 ☐ 휴일',
  '1.1 건축물 일반현황!AR17': '같은 경계로 넓어진 형제 — 운영시간 ☐ 주간(대조군)',
  // ④ 1.9 근무형태
  '1.9 자위소방대 현황!AK3': '근무형태 ☐ 상근직 (넓힌 쪽 6→7칸)',
  '1.9 자위소방대 현황!AR3': '좁아진 쪽 — ☐ 교대직 (    조   교대)',
  // ⑤ 3.5 시설이용자 — 두 칸을 옮겼다
  '3.5 피난약자 현황·계획!AU5': '시설이용자 ☐ 장애인 (넓힌 쪽 5→7칸)',
  '3.5 피난약자 현황·계획!AJ5': '좁아진 쪽 — 시설이용자 ☐ 임산부 (13→11칸)',
  '3.5 피난약자 현황·계획!BB5': '안 건드린 형제 — 시설이용자 ☐ 기타(대조군)',
  '3.5 피난약자 현황·계획!AT3': '경계를 안쪽에 품은 칸 — ☐ 영유아는 폭이 그대로여야 한다',
}
 ...
 *  한 쪽 본문 높이 한도 = `PAGE_BODY_PT`(838pt).
 *
 *  높이 예산(2026-09-21 2차 「제목 절제·사진 키움」 반영):
 *    용도 37 + 제목(런타임 100~300) + 여백 18 + **사진 400** + 여백 18 + 정보 30×2 = 533 + 제목
 *    · 제목이 하한(100)이면 633pt · 상한(300)이면 **833pt** — 한 쪽(838) 안이다.
 *  ⚠ 여유가 5pt뿐이다. 사진을 더 키우려면 `MAX_ROW_PT`(제목 띠 상한)를 같이 낮춰야 한다. */
const FP_COVER_SHEET = '표지'
/** 사진 상자 높이(pt) — 330→400(2026-09-21 2차). 제목 상한을 96→54pt로 줄여 생긴 자리를
 *  사진에 준다: **표지의 주역은 사진이지 글자가 아니다**(사용자 지시). 폭 570.7 : 높이 400 ≈ 1.43:1 */
const COVER_PHOTO_PT = 400
/** 블록 사이 여백 줄 높이(pt) */
