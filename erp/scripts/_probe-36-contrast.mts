 ...
 *        → 610/405(S7 4차).
 *  ⚠ 4차에서 조이지 않으면 개선분이 조용히 되돌아간다 — 래칫은 **내린 뒤 조여야** 래칫이다.
 *  ⚠ 여유값을 **한 번 재고 정하지 않았다**: 연속 2회 실측이 라이트 605·605 / 다크 393·**399**로
 *  다크만 6 흔들렸다. 처음 잡은 400은 여유가 1이라 다음 실행에서 그냥 붉어질 값이었다
 *  — 위 주석의 '±2~3'이 다크에서는 과소평가였다. 그래서 관측 최댓값 + 관측 편차로 다시 잡는다.
 *
 *        → **825/462(S7 5차)**. ⚠ 5차에서 **수가 커진 것은 악화가 아니라 라우트 추가**다
 *        (고객 등록 + 고객 상세 6탭 = 7라우트 신설 → 12→19라우트).
 *        같은 라우트 집합 기준으로는 **라이트 925→811 · 다크 568→448**(각 −114·−120).
 *        고객 화면군은 저장소 text-ink-faint의 78%(160건 중 125건)를 갖고 있으면서
 *        여태 **한 칸도 측정되지 않았다** — 라우트를 넓히지 않으면 고쳐도 래칫이 못 지킨다. */
const BASELINE: { light: number; dark: number } | null = { light: 825, dark: 462 }
...
try {
  ...
  for (const mode of ['light', 'dark'] as const) {
    ...
    const routes: Array<[string, string]> = [
      ...
      // ── S7-1 **5차**(2026-08-30) — 고객 화면군. 저장소의 text-ink-faint 160건 중
      //    125건이 components/customers/ 에 있는데 여태 **한 칸도 측정되지 않았다**.
      //    고객 상세는 탭 셸이라 탭마다 다른 컴포넌트가 붙는다 — 탭을 안 돌면 그 탭의
      //    컴포넌트는 통째로 사각지대다. 그래서 탭을 URL로 하나씩 연다.
      //    ⚠ `?tab=`을 **클릭**으로 돌면 안 된다: 같은 경로 Link는 서버 재렌더를 안 깨워
      ...
      [`/customers/${cust}?tab=billing`, '고객 청구·수금'],
    ]
