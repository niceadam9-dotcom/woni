...
//   그래서 여기서는 **실제 조립 함수**를 부른다.
{
  ...
  /* 🎯 사항 = 「결과참조」 · 일자 = 총 이행기간(불량 있는 구분만). 이 자리는 다섯 번 바뀌었다:
   *     ① 구분마다 다른 날짜 → ② 09-09 총 기간을 7행에 복제(같은 기간 여덟 번) →
   *     ③ 09-10 일자를 통째로 「결과참조」 → ④ 09-11 **사항·일자를 맞바꿈 + 7행 전부 기간** →
   *     ⑤ 09-20 **이상없음·해당없음 행은 기간 제외**(사용자 지시 「별지 10~11에서 이상없음과
   *        해당없음은 이행기간이 조회되지 않도록」 — 엑셀 21칸 동일).
   *   날짜가 붙는 행은 **불량 있는 구분으로 한정**된다(이 픽스처에선 2행). 7행 전체에 붙기
   *   시작하면 ④로 되돌아간 것이니 아래 dated 개수와 음성 대조가 함께 붉어져야 한다.
  ...
  /* 🚨 2026-09-20 **세 번째 계약 교체**(⑤판 — 지우지 않고 반대 방향으로 세운다).
   *   09-11 아침 판: 「불량 있는 구분만 기간」 → 09-11 확정 판: 「7행 전부 기간」(두 안을
   *   미리보기로 보고 택함) → **09-20 사용자 지시로 다시 갱신**: 「별지 10~11에서 이상없음과
   *   해당없음은 이행기간이 조회되지 않도록」(엑셀 21칸 동일). 이행할 것이 없는 설비에 기간이
   *   서면 이행기간이 있는 것처럼 읽힌다. 문구 행은 `—`(Q-5 b안 폴백)로 돌아간다. */
  for (const g of notes) {
    const seg = bodyOnly.slice(bodyOnly.indexOf(`>${g.group}<`))
    const cell = seg.slice(0, seg.indexOf('</tr>'))
    ok(!cell.includes(TOTAL_TEXT) && cell.includes('—'),
      `🎯 ${g.group}(${g.content}) 일자 칸은 「—」 — 총 이행기간이 서지 않는다(09-20 사용자 지시)`)
  }
  ...
  // 일자 칸 = **총 이행기간, 불량 있는 구분(결과참조 행)만** (2026-09-20 사용자 지시 — ⑤판)
  const dated = realRows.filter(r => !r.isNote)
  ok(dated.length === 2 && notes.length === DEFECT_GROUPS.length - 2,
    `일자 대상 ${dated.length}행 + 문구 행 ${notes.length}행 = 7행(양성·음성 표본 선단언)`)
  /* 🚨 **생산자 층에서도** 잰다 — 렌더만 보면 부족하다. 09-11 아침 판의 교훈:
   *   렌더가 isNote에 무조건 `—`를 찍던 시절엔 **생산자에 period가 실려도 화면이 조용해서**
   *   「7행 전체」 변이가 스위트를 통과했다. 렌더는 지금도 period 우선이므로(값이 실리면 찍는다),
   *   **생산자가 문구 행을 비웠는가**를 직접 묻는 이 단언이 이 계약의 정본이다. */
  ok(dated.every(r => r.period !== '') && notes.every(r => r.period === ''),
    '🎯 (생산자) period는 불량 있는 구분에만 — 이상없음·해당없음 행은 빈다(09-20)')
  /* 🚨 일수는 기간과 **한 벌**(2026-09-14 엑셀 21칸 정렬) — 기간이 비는 행은 일수도 빈다.
     한쪽만 비우면 `(총 N 일)` 꼬리가 허공에 뜬다(E10-8). */
  ok(dated.every(r => r.days !== '') && notes.every(r => r.days === ''),
    '🎯 (생산자) days도 같은 조건 — 문구 행에 일수만 남지 않는다')
 ...
 *   `DEFECT_FOLD_TEXT.refer`(11호 축이 쓴다)에 매치돼 음성 단언이 헛도는 것을 막는다. */
{
  ...
  /* 🚨 2026-09-20 계약 교체(두 번 뒤집힌 자리 — 09-11 아침 「제외」 → 09-11 확정 「제외 없음」
   *   → 09-20 사용자 지시로 「제외」 복귀). 조립본(annexPlanRows)이 이상없음·해당없음 행의
   *   기간을 비우므로 덧칠도 같은 행을 건너뛰어야 한다 — 한쪽만 다르면 자동 산출이 있는
   *   회차와 수기 보정만 있는 회차가 서로 다른 표를 인쇄한다. */
  ok(/isNote\s*\?\s*r\s*:/.test(overlay),
    '🎯 덧칠이 isNote 줄을 건드리지 않는다(조립본과 같은 축 — 09-20)')
