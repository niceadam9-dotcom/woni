...
const MUTANTS = [
  ...
  { name: 'W1 기본정보 탭의 넓게 쓰기를 뺀다', file: PAGE,
    from: "        wideKeys={['info', 'buildings', 'contacts', 'billing', 'history']}", to: '', expect: '넓게(wideKeys)' },
  ...
  { name: 'V1 ★ 저장 버튼을 다시 「고쳐야만」 보이게 — 사용자가 「저장버튼이 없네?」라 한 그 화면', file: INFO,
    from: '        {canManage && (\n          <SaveBar testId="info-save-bar"',
    to: '        {canManage && isDirty && (\n          <SaveBar testId="info-save-bar"', expect: '늘** 있다' },
  { name: 'V1b ★ 부품이 변경 없을 때 저장 줄을 숨긴다 — 모든 탭에서 저장 버튼이 사라진다', file: KF,
    from: '  const canSave = alwaysEnabled || dirty\n', to: '  const canSave = alwaysEnabled || dirty\n  if (!dirty) return null\n',
    expect: '저장 줄 부품' },
  { name: 'Q1 서식 1.2가 옛 인라인 버튼으로 돌아간다(공용 부품 이탈)', file: 'src/components/customers/plan-form12.tsx',
    from: '<SaveBar dirty={dirty} pending={isPending}', to: '<SaveBarOld dirty={dirty} pending={isPending}', expect: '공용 SaveBar를 쓴다' },
  { name: 'Q2 1.4 저장 testid를 잃는다 — 검사·E2E가 버튼을 못 잡는다', file: 'src/components/customers/plan-form14.tsx',
    from: '<SaveBar saveTestId="form14-save"', to: '<SaveBar', expect: '그대로 옮겨졌다' },
  { name: 'Q3 청구·수금을 좁게 둔다', file: PAGE,
    from: "wideKeys={['info', 'buildings', 'contacts', 'billing', 'history']}", to: "wideKeys={['info', 'buildings', 'contacts']}",
    expect: '청구·수금·이력 탭도 넓게' },
  ...
  { name: 'P2 관계인 탭을 넓게 쓰지 않는다', file: PAGE,
    from: "wideKeys={['info', 'buildings', 'contacts', 'billing', 'history']}", to: "wideKeys={['info', 'buildings', 'billing', 'history']}", expect: '세 탭 모두 넓게' },
]
