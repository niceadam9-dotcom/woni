...
const MUTATIONS = [
  ...
  /* 🔁 「여러 줄 칸은 제외」 규칙은 2026-09-16에 **폐지**됐다(여러 줄에도 단다 — 사용자 지적 image-1).
   *   변이를 지우지 않고 **반대 방향으로 옮긴다**: 도로 빼면 전기차충전소를 못 누르게 된다.
   *   ⚠ 지우기만 하면 「여러 줄을 정말 다는가」를 아무도 안 묻는다. */
  {
    name: '[여러 줄] 여러 줄 칸을 도로 뺀다(전기차충전소를 못 누르게 된다)',
    from: '    const lineOf = (i: number)',
    to: "    if (label.includes('\\n')) continue\n    const lineOf = (i: number)",
  },
  {
    name: '[여러 줄] 줄 번호를 늘 0으로(상자가 첫 줄에 겹쳐 쌓인다 — 실사고)',
    from: 'lineIndex: lineOf(at[i])', to: 'lineIndex: 0',
  },
  ...
  /* ── 다중상자(`□ 유 □ 무`) 축 — 2026-09-14 ─────────────────────────────────────── */
  {
    ...
    from: 'if (!offs && [...perLine.values()].some(n => n > 1)) continue',
    to: 'if ([...perLine.values()].some(n => n > 1)) continue',
  },
  {
    name: '[다중상자] 실측 오프셋을 버리고 0으로(상자가 전부 첫 자리에 겹친다)',
    from: 'offsetPx: offs?.[i] ?? 0,',
    to: 'offsetPx: 0,',
  },
  ...
  {
    name: '[들여쓰기] 보정을 0으로(컨트롤이 상자보다 16px 왼쪽에 선다 — 43·44회차 배포본의 상태)',
    from: 'export const TEXT_INSET_PX = 13', to: 'export const TEXT_INSET_PX = 0',
  },
  /* 🚨 **실사고 변이**(2026-09-16). 실측 기준선(16px)에 「정확히」 맞추면 상자 뒤에 공백이 없는
   *   라벨에서 여유가 0~1px이 되어 사용자 화면에서 글자를 덮는다. 렌더 없는 검사로는 여유를 못 재므로
   *   **「편향이 지워졌는가」**를 문다 — 이게 없으면 다음 사람이 「실측값이 16인데 왜 13이지?」 하며 되돌린다. */
  {
    name: '[여유] 들여쓰기를 실측 기준선에 딱 맞춘다(여유 0~1px — 화면에서 글자를 덮는다)',
    from: 'export const TEXT_INSET_PX = 13', to: 'export const TEXT_INSET_PX = 16',
  },
  /* 🚨 **실사고 변이**(2026-09-16). 앵커를 짤 때 그 칸의 **첫** 상자를 보는 바람에 여러 줄 칸의
   *   상자가 첫 줄에 겹쳐 쌓였고, 2·3번째 줄은 「상자 글자는 지워졌는데 컨트롤이 없는」 칸이 됐다.
   *   판정 축 단언(줄 번호 0·1·2)은 **전부 초록**이었다 — 잡은 것은 산출물 세로 자리 단언뿐이다. */
  {
    name: '[여러 줄·실사고] 앵커가 칸의 첫 상자를 본다(줄마다 안 갈리고 첫 줄에 쌓인다)',
    from: '(b.lineIndex - (b.lineCount - 1) / 2)', to: '(c.lineIndex - (c.lineCount - 1) / 2)',
  },
]
