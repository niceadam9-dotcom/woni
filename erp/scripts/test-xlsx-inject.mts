...
console.log('[9] 현5 불량 세부 · 계획서 폐포')
{
  ...
  /* ★ 2026-09-11 — 계획서!H12는 더 이상 **거울이 아니다**. 종전엔 `={현5!C4}` 단일 참조라
   *   이행 폐포가 캐시를 옮겨 줘야 인쇄가 됐다(LO는 재계산을 안 한다, D-9). 이제 앵커
   *   `planContent12`가 **값을 직접 쓰고 수식을 끊는다** — 폐포에 기대지 않으므로 더 튼튼하다.
   *   ⚠ 이 픽스처는 `applicableGroups` **미공급**이라 접기를 타지 않는다. 그 경로의 계약은
   *     「현5와 같은 텍스트」이고(종전 거울과 같은 결과), 접기 경로(「결과참조」)는
   *     test-applicable-surfaces가 네 표면으로 본다. */
  const h12 = cellOf(r9.bytes, '계획서', 'H12')
  check('계획서!H12 — 미공급 경로는 현5!C4와 같은 텍스트를 직접 받는다', String(h12?.v ?? '') === c4,
    `v=${JSON.stringify(String(h12?.v ?? ''))} / 현5!C4=${JSON.stringify(c4)}`)
  check('계획서!H12 — =현5!C4 수식이 끊겼다(Excel 재계산 되돌림 차단)',
    typeof h12?.f !== 'string', `f=${h12?.f ?? '(없음)'}`)
