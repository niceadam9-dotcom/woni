...
const steps: Step[] = [
  ...
  // 1.4 확인 게이트(소방계획서_49 §6) — 경고와 **완료 보류**.
  // 🚨 등재 이유: 관문이 ⓑ경고만이라 지나칠 수 있고, 전부 ／인 회차는 따라잡기도 안 돈다
  //   (／는 트리거가 아니다). 그때 대장이 비면 분모가 0이라 안전장치가 동시에 침묵하므로
  //   **이 보류가 유일한 그물**이다. 핵심은 음성 — [C] 확인만 눌렀으면 설비 0건이어도 안 막는다.
  //   [E]가 보류 축이 OR로 실제 합쳐지는지 센다(`{required:0}`으로 대신하면 조용히 안 걸린다).
  { name: '1.4 확인 게이트·완료 보류',  cmd: 'npx tsx --conditions=react-server scripts/test-facility-verify-gate.mts' },
  ...
  // 서식 첨부 이미지 슬롯(ImageSlot) — 창구 통일·다운로드·화살표 주석·표지 사진 가져오기.
  // 🚨 등재 이유: 2026-09-09 구현 당시 이 파일이 타 세션 `M`이라 등재를 미뤘고, 그대로 **잊혔다** —
  //   다섯 달 치 축(붙여넣기·드롭·다운로드·재편집)이 일괄 검사 밖에 있었다.
  //   판정은 육안이 못 가르는 자리에 건다: 합성본 **픽셀**에 배경(초록)과 화살표(빨강)가 둘 다 있어야 하고
  //   (빨강만=배경 유실, 초록만=화살표 유실, 흰색뿐=canvas 오염), 표지 가져오기는 경로가 아니라
  //   **바이트 동일성**으로 본다(경로만 맞고 내용이 다른 복사를 가른다).
  //   7절이 8절(삭제)보다 **먼저** 와야 한다 — 순서가 바뀌면 '옛 원본·좌표가 따라오지 않는가'가
  //   빈 상태에서 돌아 공허 통과한다. 그래서 그 전제 자체를 단언으로 박아 뒀다.
  //   변이 프로브: `node scripts/_mutate-route-cover.mjs` (5/5 빨강 실측 2026-09-14)
  { name: '이미지 슬롯·표지 가져오기(E2E)', cmd: 'npx tsx scripts/test-image-slot.mts', needServer: true },
  ...
  //   변이 프로브: `node scripts/_mutate-fireplan-images.mjs` (16/16 빨강 실측 2026-09-14 —
  //   그중 M11은 「상자를 잘못 재도 초록」이던 사각을 드러내 단언을 하나 더 낳았다)
  // 2026-09-14 확대 — 1.3 「건축물 위치」 상자의 임자가 **표지 건물 사진**으로 바뀌면서(약도가
  //   아니다·사용자 확정) [7]절에 **PDF 축**이 생겼다. 엑셀만 고치면 나머지 106건이 초록인 채로
  //   PDF가 옛 약도를 계속 인쇄한다(M16이 그 실수를 재현한다).
  { name: '소방계획서 사진 상자',      cmd: 'npx tsx --conditions=react-server scripts/test-fire-plan-images.mts' },
  // 그림 **배정 규칙**(2026-09-14) — 어느 사진이 어느 자리에 인쇄되는가. 위 검사가 「상자에 잘 앉는가」를
  //   본다면 이쪽은 **「거기 올 것이 그것인가」**를 본다. 규칙이 assembleFirePlan 한가운데 DB 왕복과
  //   섞여 있어 여태 단언이 0건이었고, 그 틈으로 「진입 경로도 = 표지 건물 사진」이 버튼 뒤에 갇혀 샜다.
  // ⭐ 판정축은 자리가 아니라 **바이트 동일성(같은 path)**이다 — 자리만 물으면 엉뚱한 사진도 초록이다.
  // ⭐ 대역은 **물러날 줄 알아야** 규칙이다(제 경로도가 있으면 표지가 안 온다) — 음성 단언이 짝이다.
  //   변이 프로브: `node scripts/_mutate-fireplan-route-fallback.mjs` (6/6 빨강 실측 2026-09-14 —
  //   M3·M6이 「약도를 대역으로 써도 초록」·「SINGLE_KINDS를 지워도 초록」이던 사각을 드러냈다)
  { name: '소방계획서 그림 배정 규칙', cmd: 'npx tsx scripts/test-fire-plan-image-refs.mts' },
  ...
  // 정기(monthly)는 점검표 입력을 하지 않는다(2026-09-14) — 데이 패널 계획 행에서 ▶[시작·완료]와
  // [점검 보기] 두 입구를 함께 없앤 축. 핵심 단언은 「정기에서 껐는가」가 아니라 **「일반에서
  // 끄지 않았는가」**다. 서버 없이 순수 함수 + 배선으로 판정한다(변이 5/5 실측).
  { name: '달력 계획 행 — 정기 점검 입구 폐지', cmd: 'npx tsx scripts/test-calendar-plan-row-entry.mts' },
  // "지평리56이 달력에서 조회가 안 된다"(2026-09-14) — 결함이 **둘**이었고 이 축 검사는 0건이라
  // 둘 다 전 스위트 초록인 채로 운영에 나가 있었다: ①서버가 자체점검 계획을 아예 안 실었고
  // (`.in(plan_type)`에 special_* 누락 → 점검이 시작되기 전까지 그 고객은 어느 칸에도 없다)
  // ②정기 집계 칩이 1건뿐인 날에도 이름을 가렸다. 핵심 단언은 「폈는가」가 아니라 **「뭉쳐야 할
  // 때 그대로 뭉치는가」**다 — 음성만 물으면 `groups:[]` 한 줄로 전부 초록이 되고 그건 하루
  // 100건 "+N개 더 보기" 폭발로 되돌아가는 것이다. 서버 없이 순수 함수 + 배선(변이 9/9 실측).
  { name: '달력 계획 표시 — 자체점검 적재·정기 펴기', cmd: 'npx tsx scripts/test-calendar-plan-visibility.mts' },
  // 별지서식 회차 배지(2026-09-14 사용자 신고: 「예정 지연 172일 ⚠」이 오해를 부른다).
  // 실측하니 한 고객 문제가 아니었다 — 자체점검 777건 중 **292건(37.5%)**이 붉은 배지였고
  // 중앙 151일·최대 255일이었다. 37.5%에 뜨는 경고는 경고가 아니고, 「151일 지연」은
  // **법 위반 151일**로 읽힌다. 법정 시기는 **달** 단위라(시행규칙 [별표 3]) 일 단위는
  // 법에 없는 정밀도이고 같은 달이면 애초에 늦은 것이 아니다 → 「이달 예정」/「N개월 경과·미실시」.
  // 🎯 핵심 단언은 「붉은색을 없앴는가」가 아니라 **「없애면 안 될 붉은색을 남겼는가」**다
  //    ([A8][B2] — 시작된 점검의 overdue는 유일하게 붉은 채로 남는다). 회차(N차) 표시 폐지도
  //    같은 축에서 고정하되, **DB sequence_num은 지우지 않았음**을 [C4][C5]가 음성으로 지킨다
  //    (계획 생성 멱등 축이자 1차/2차로 일반관리·종합·작동을 가르는 의미 축이다). 변이 10/10 실측.
  { name: '별지 회차 배지 — 달 단위·회차 폐지', cmd: 'npx tsx scripts/test-annex-round-pill.mts' },
  // 자체점검 2차(작동)가 **해를 넘긴다**(2026-09-14). 종전 산식은 감긴 달을 같은 해에 두어
  // 기산월 7~12인 종합 대상(활성 305명 중 8명)의 2차가 1차보다 **앞선 달**에 앉았고, 첫 해의
  // 2차는 짝이 될 종합이 없는 **고아**라 태어날 때부터 과거였다(지평리56 2026-03-26).
  // 🎯 핵심 단언은 「해를 넘기는가」가 아니라 **「정상 상태의 날짜가 그대로인가」**([D0]) —
  //    6개월 간격은 종전과 같고 사라지는 것은 첫 해 고아 하나뿐이다. [B4][C3]이 과잉 삭제 대조.
  // 🚨 달 산식이 **두 벌**이었다(plan-anchor + 생성기 인라인 사본) → [E1]이 사본 부활을 막는다.
  // 🚨 [E0]은 **계측기 자기 검사**다 — codeOnly가 CRLF에서 주석을 한 줄도 안 걷어내던 것이
  //    이 검사의 거짓 빨강으로 드러났다(scripts/_code-only.mts 참조). 변이 13/13 실측.
  { name: '자체점검 2차 해넘김·고아 폐지', cmd: 'npx tsx scripts/test-special-slot-year.mts' },
