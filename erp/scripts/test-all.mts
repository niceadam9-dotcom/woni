...
const steps: Step[] = [
  ...
  // 과거·오늘 점검일자 = 점검 사실 (2026-09-20 하늘촌 신고) — 입력값 **그대로**(영업일 보정 없음)
  // 1차 scheduled_date가 되고 즉시 시작된다(시작일=그 날짜). 종전엔 보정이 입력값을 덮고(09-18→09-21)
  // 시작은 당일 크론뿐이라(창 D-3~D0) 과거 날짜는 영영 시작되지 않았다. 미래 날짜 불가침·배선까지 문다.
  { name: '과거 점검일자 즉시 시작',      cmd: 'npx tsx scripts/test-anchor-past-start.mts' },
  ...
  // 그 워크북을 받는 버튼의 **이름이 한 벌인가**(2026-09-21). 위 검사들은 문서의 *속*만 보고
  // 「사용자가 그걸 무엇이라 부르는 버튼으로 받는가」는 아무도 안 봤다 — 그래서 같은 산출물이
  // 표면마다 「엑셀」·「엑셀로 받기」로 갈라져 있었고, 옆에 [소방계획서 엑셀]이 뜨는 회차 탭에서
  // 특히 안 읽혔다. 문자열 존재가 아니라 **단일 원천 참조**를 문다(표면이 각자 적으면 또 갈라진다).
  { name: '결과보고서 엑셀 라벨 단일성', cmd: 'npx tsx scripts/test-workbook-label.mts' },
  ...
  // 제목 **크기** 계약(2026-09-21 2차 「제목 절제」) — 글꼴 검사는 템플릿의 출발값만 보고
  // 런타임이 정하는 크기는 안 본다. 1차에서 상한을 96pt로 올렸더니 짧은 이름이 상한을 그대로
  // 받아 제목이 사진과 맞먹었다(용문3 96pt·image-26). 여기가 서버 없이 넷을 문다:
  // 상한 · **짧은 이름은 한 줄** · 폭 초과 0 · 한 쪽(띠 상한을 예산에 맞댄 최악의 경우 포함).
  { name: '소방계획서 표지 제목 크기', cmd: 'npx tsx scripts/test-cover-title-size.mts' },
  ...
  // 엑셀 고지 → 클릭 칩. 🚨 고지가 말하는 번호는 **시트 번호**라 화면 노드 키와 다르다 —
  //   1.15의 뒤를 자르면 1.1(일반현황)이 나오는데 그건 **실재하는 엉뚱한 화면**이다.
  //   여기가 붉으면 사용자를 조용히 딴 화면으로 보내고 있다는 뜻이다(고장보다 나쁘다).
  { name: '엑셀 고지 칩',              cmd: 'npx tsx --conditions=react-server scripts/test-fire-plan-notice.mts' },
  ...
  // 🚨 격자 → px 환산의 **정답지 검사**(2026-09-21). 아래 사진 상자 검사는 기대값을 제품과
 //   같은 환산 함수로 만들어, 그 함수가 틀리면 **함께 틀려 늘 초록**이다 — 실제로 표지 사진이
 //   160px 밀려 나가는 동안 「가운데」가 통과했다(변이 3종 전부 생존해 그 눈멂을 실증).
 //   여기는 산식 밖의 기준(Excel COM 실측 780px)을 정답지로 박아 그 구멍을 닫는다.
  { name: '엑셀 격자 px 환산(정답지)', cmd: 'npx tsx scripts/test-xlsx-geometry.mts' },
  ...
  // 🚨 등재 이유(2026-09-15): 이 스위트는 `cf139d0`(2026-09-11)에 만들어졌는데 **게이트에 없었다.**
  //   그 사이 [+ 건물 등록] 버튼이 사라진 것을 스위트가 한 번도 알리지 않았고, 사용자가
  //   「추가 건축물등록 어디로 갔어」로 신고할 때까지 아무도 몰랐다 — 위 [C]와 **같은 종류의**
  //   구멍이다(검사는 있는데 아무도 안 돌린다). 무서버 소스 검사라 게이트 비용도 없다.
  //   지키는 계약 셋: 0동 고객 금지(§10-2 ①) · 버튼이 조건에 가려 숨지 않음([H6]) ·
  //   2동이면 표를 안 감춤([I2], 다동 고객이 동을 고르는 유일한 경로).
  { name: '건물 1동 불변식 + [+ 건물 등록] 배선', cmd: 'npx tsx scripts/test-49-building-always.mts' },
  ...
  /* 시설현황 상자 규칙(2026-09-16) — 서식 1.1 12~16행(승강기·주차장·계단)을 켜고 끄는 술어 한 벌.
   * ⚠ 이 축도 검사가 **0건**이었고, 그 사이 실제 결함이 인쇄되고 있었다: 판정이 `!!txt(v)`라
   *   문자열 `'0'`이 상자를 켰고(송학떡집 옥외계단), 계단 입력구가 **세 화면**으로 갈라져
  ...
  // 2026-09-14 확대 — **3순위 「법정 기본」**이 생겼다(C-2절). 2순위(자동)의 원천인 불량별 계획
  //   시작·종료일 입력이 2026-09-11에 폐지돼, ④에서 손으로 넣지 않으면 법정 서식의 이행조치기간이
  //   **반드시** 공란으로 제출된다(스테이징 실측: 불량 있는 7회차 중 5회차 공란·수기 입력 0건).
  // ⭐ 지어내는 값이라 경계가 핵심이다 — 「깐다」보다 **「깔면 안 될 때 안 까는가」**(불량 0건·기산일
  //   없음)와 **서열 맨 아래인가**를 묻는다. 배선 단언은 두 표면이 **같은 기산일**을 넘기는지 본다.
  //   변이 프로브: `node scripts/_mutate-action-period-legal.mjs` (12/12 빨강 실측 2026-09-20 —
  //   M7이 「덧칠 일수를 비워도 초록」이던 사각을 드러내 모양 단언을 값의 출처 단언으로 바꿨고,
  //   M10~12가 ⑤판 「이상없음·해당없음 기간 제외」의 세 표면(조립본·덧칠·엑셀 21칸)을 따로 문다)
  { name: '총 이행기간 → 엑셀·11호 착지', cmd: 'npx tsx --conditions=react-server scripts/test-annex-total-period.mts' },
  // 별지 보고일 ← 소방서 제출 기록 (2026-09-20 사용자 요구 — 「문서의 날짜를 소방서 제출일로」).
  // 등재 이유: 이 축의 회귀는 화면에서 안 보인다 — 폴백이 오늘로 떨어져도 문서는 그럴듯한 날짜를
  //   인쇄하고, 제출을 기록한 뒤 다시 뽑은 문서만 조용히 다른 날을 말한다. 종전엔 주석 두 곳이
  //   「④ 제출일 가지」를 약속하면서 **구현은 없었다**(select만 하고 안 쓰는 형태 — 검사 0건이라 산 것).
  // ⭐ 핵심은 축 분리다: 9·10호=④, 11호=⑥ — 「제출 기록을 쓴다」만 물으면 축이 섞여도 초록이 된다.
  //   변이 프로브: `node scripts/_mutate-annex-report-date.mjs`
  { name: '별지 보고일 ← 제출 기록', cmd: 'npx tsx --conditions=react-server scripts/test-annex-report-date.mts' },
  ...
  // 공통 서술 라이브러리 목록 미리보기가 **대표 칸**을 띄우는가 — 서버·DB 불필요, 순수 함수.
  // 이 결함은 화면상 멀쩡해 보인다(문장이 하나 떠 있으니 비지도 깨지지도 않았다). 다만 엉뚱한
  // 칸이라 "이름만으로 항목을 구분 못 한다"를 보완하라던 미리보기가 제 일을 안 한다.
  // 뿌리는 **Postgres jsonb가 키를 「길이→바이트순」으로 재정렬**하는데 코드가 객체 순서를
  // 믿은 것이다(2026-09-21 실측 7건 중 2건: 팀별임무가 지휘통제 대신 응급구조, 공사·정비가
  // 내용 대신 비고). 나머지 4건은 **우연히** 맞았을 뿐이라 상시 고정이 필요하다.
  // 급소: 본문을 선언 순서로 만들면 결함 코드로도 초록이다 → jsonb 순서로 재현하고 순열까지 돈다.
  { name: '공통문구 미리보기 대표 칸',  cmd: 'npx tsx scripts/test-plan-text-preview.mts' },
  // 공통문구 전역 관리 화면(소방계획서_21 R1-12) — 위 순수 검사가 「대표 칸」 하나를 묻는다면
  // 이쪽은 화면의 계약 전부를 왕복으로 본다: ⭐/○ 집계 · version 의미론(본문이 바뀔 때만 +1,
  // 이름만 바꾸면 불변) · 섹션당 ⭐ 1개 · 복제 시 원본 불변 · **불러오기에 고객 고유 필드
  // (일시·장소) 미혼입** · 소프트 삭제 후에도 가져간 고객 입력 보존.
  // 🚨 **여태 test-all에 없던 고아였다**(2026-09-21 실측: test-* 189개 중 49개가 미등록).
  //   고아라서 「기본 문구」 8섹션 등록으로 전제가 깨진 것을 아무도 알려주지 않았다. 돌려 보니
  ...
  // 점검일자 변경 가드(2026-09-22) — 서버 불필요(순수 판정 + 배선).
  // 지키는 것은 「사용자가 불편한가」가 아니라 **제출한 서류와 ERP가 어긋나는가**다.
  // 2단계(배치신고)부터는 협회·소방서에 **이미 나간 날짜**가 있어 기산일을 움직이면 갈라지고,
  // 그 갈라짐은 인쇄물을 열어 보기 전까지 화면상 멀쩡해 보인다.
  // 🚨 급소 둘: ①빈 배열을 통과시키면 조회 실패가 「마음대로 옮겨도 됨」이 된다
  //   ②판정을 **표시 축**(불량 0이면 ⑤⑥ 숨김)으로 세면 숨겨진 단계의 완료를 못 보고 통과시킨다.
  { name: '점검일자 변경 가드',         cmd: 'npx tsx scripts/test-inspection-date-change.mts' },
  // 달력에서 고객 등록(2026-09-22) — 서버 불필요(배선). 등록은 이미 달력 일정을 만들므로
  // 등록하면 그 자리에 칩이 뜬다. 여기서 지키는 것은 「버튼이 뜨는가」가 아니라 셋이다:
  //   ① **폼이 복제되지 않았는가** — customer-new-client(877줄)에 필수 6칸·중복검사·대장조회가
  //      들어 있다. 「달력용 간단 폼」을 새로 짜면 달력 등록만 중복 고객을 허용하게 된다.
  //   ② **권한으로 버튼 자체를 가리는가** — 안 가리면 눌러 봐야 서버가 던진다.
  //   ③ **왕복이 닫히는가**(2026-09-22 계약 교대 — 모달 → `/customers/new` 페이지). 「입력 다 하고
  //      사이드바로 복귀, 만약 사이드바에서 왔다면」의 조건을 코드가 따로 판정하지 않는다:
  //      데이 패널이 열려 있으면 주소에 `day=`가 실리고 닫혀 있으면 안 실린다 — **주소가 조건이다.**
  //      네 고리(㉠day 기록 ㉡복귀주소에 싣기 ㉢서버 되읽기 ㉣떠나기 전 패널 안 닫기) 중 하나만
  //      끊겨도 「돌아왔는데 달력만 있다」가 된다.
  //   ④ **기존 /customers/new가 그대로인가** — from 미제공 시 고객 상세로 가는 폴백이 살아 있어야
  //      한다. 사라지면 사이드바 밖에서 등록한 사람은 등록하고 아무 데도 못 간다.
  { name: '달력 고객 등록 배선',        cmd: 'npx tsx scripts/test-calendar-new-customer.mts' },
  // 「등록했는데 단계가 없다」를 없애는 축(2026-09-22) — 서버 불필요(순수 + 배선).
  // 점검일자가 과거·오늘이면 1차가 즉시 시작돼 1~4단계가 생기고, 미래면 계획만 생긴다.
  // 달력에서 날짜를 짚어 등록하게 되면서 **미래를 고를 문이 열렸는데**, 종전엔 그 결과가
  // 화면으로 올라오지 않아(호출부가 applied를 버렸다) 「단계가 생겼습니다」가 거짓이 될 수 있었다.
  // 🚨 급소 셋: ①경계는 **오늘 포함** ②판정식이 한 벌(폼·서버 공유) ③그래서 **순수 모듈**에 있어야
  //   한다 — inspection-start는 next/cache를 들여와 클라이언트가 못 쓴다(tsc 0인데 런타임이 깨진다).
  { name: '과거·오늘 점검일자 축',      cmd: 'npx tsx scripts/test-past-anchor.mts' },
  // 결과보고서 고지 분류(2026-09-22) — 서버 불필요(순수). 화면 변경 0의 기반 모듈.
  // 고지를 「채우러 가기」 입구로 바꾸기 전에(R5) **채울 수 있는 것과 없는 것을 갈라야** 한다.
  // 안 가르면 사용자가 **못 고치는 것**(서식 행수 상한: 허브 7행·엑셀 4행…)을 고치러 헤맨다.
  // 🚨 급소: ①상한을 fixable로 ②안내(문서번호 자동 제안)를 누락으로 ③모르는 문장에 목적지를
  //   지어내기(fire-plan-notice의 「3.5층창고」 사고) ④표가 조용히 썩기.
  ...
  // 🚨 2026-09-22 **계약 교대** — 달력은 보고서 고지를 **그리지 않는다**(사용자 지시:
  //   「보고서엑셀 클릭시 『채우면 다음 발행에 반영됩니다』 내용 모두 없애 달라」). 실측이 뒷받침한다:
  //   자체점검 31건 **전건**에 고지가 떠, 400px 사이드바가 버튼 한 번에 칩 묶음으로 덮였다.
  //   낡은 단언(쪽지·from·workbookFixHref·복귀 띠)은 지우지 않고 **음성으로 갈아끼웠다** —
  //   되살아나면 사용자가 뺀 화면이 조용히 돌아오는 것이므로 여기서 멈춰 선다.
  // 🚨 급소 셋: ①`owns = !onNotice && !onError`가 무너지면 버튼이 **자기 고지를 대신 그린다**
  //   (겉보기엔 아무것도 안 바꾼 듯 초록) ②오류(`onError`)까지 걷으면 다운로드 실패가 조용해진다
  //   ③소방계획서 고지는 **남겨 둔 축**이라 파일 전체에 DocNoticeList를 물으면 영영 빨강이다.
  { name: '달력 보고서 고지 제거',      cmd: 'npx tsx scripts/test-calendar-workbook-notice.mts' },
  // 달력의 소방계획서 고지 칩(2026-09-22) — 서버 불필요(순수 + 배선).
  // 🚨 실측(활성 고객 12명 전건)이 이 축의 모양을 정했다: 실제 고지 조각 17종이 **거의 전부
  //   미입력 라벨 그 자체**였고(수신기위치·구조·지붕·선임일…), **시트 번호가 있는 조각은 0종**이었다.
  //   즉 parseFirePlanNotice의 노드 매핑만으로는 칩이 하나도 안 생긴다 — 라벨 표가 이 축의 본체다.
  // 🚨 급소: 모르는 라벨에 「1.1로」 폴백을 두면 그 순간 오배송이 된다(3.5층창고 사고와 같은 부류).
  { name: '달력 소방계획서 칩',         cmd: 'npx tsx scripts/test-calendar-fireplan-notice.mts' },
  ...
  // 🚨 재발 방지 불변식(2026-09-21) — 「사람이 체크한 것은 문서에도 체크된다」를 **배선 전수**로 문다.
  //   같은 증상이 최소 네 번 재발했고(하위 ck(false) 하드코딩 · 앵커 0칸 1.4/1.8/1.10.1 · 기타 3종
  //   응답 축), 넷 다 tsc와 소스 단언을 통과했다 — 그래서 **받은 파일을 열어** 판정한다.
  //   여기가 붉으면 새로 늘린 행의 체크 축이 대장이 아니거나, 그 칸이 아예 안 채워지고 있는 것이다.
  { name: '문서 체크칸 배선 전수(E2E)',  cmd: 'npx tsx scripts/test-doc-check-wiring.mts',   needServer: true },
  // 위 가드가 사용자를 점검표로 보낸 **뒤**를 문다(2026-09-21). 종전엔 이동하는 순간 「엑셀을
  // 받으려던 것」이 사라져, 입력을 마치고 돌아와도 같은 버튼을 다시 눌러야 했다. 무서버 축이라
  // 여기 E2E들과 달리 pre-push에도 얹힌다 — 규칙(one-shot·TTL)과 **배선**을 함께 문다.
  { name: '발행 의도 표식·복귀 발행',     cmd: 'npx tsx scripts/test-pending-doc-intent.mts' },
  // 위가 규칙·배선이라면 여기는 **실브라우저만 답할 수 있는 것**을 문다: 쪽지가 전체 이동을 건너
  // 살아남는가 · 누르지 않았는데 발행되는가 · **제스처 없는 a.click()이 실제로 먹히는가**.
  // 마지막 줄은 코드로는 알 수 없고 막혀도 조용하다 — 폴백 배너를 넣은 이유이자 실측할 이유다.
  { name: '복귀 발행(E2E)',              cmd: 'npx tsx scripts/test-resume-publish-e2e.mts', needServer: true },
  ...
  // ⚠ 실고객을 쓰므로 프로브가 **원상복구**까지 한다 — `process.exit` 금지 규약이 거기 걸려 있다.
  { name: '1.14.1 홍보 계획 왕복(E2E)',  cmd: 'npx tsx scripts/_probe-1141-live.mts',          needServer: true },
  // 4단계 원격 편집(소방계획서_50 §10-2b, 2026-09-18) — 빈칸 보고의 미입력 칸이 **다른 절**
  // 소유면 「입력: <절 이름>」 링크로 보낸다(1.15 칸은 1.10이 소유). 단위 검사(전수·차분)는
  // 선언이 옳은지만 묻고, **사용자가 실제로 누르는 것**은 이 E2E만이 본다 — 링크가 붙는가·
  // 자기 절로 보내지 않는가·눌렀을 때 그 절과 카드 앵커가 열리는가.
  { name: '빈칸 원격 입력처 링크(E2E)',  cmd: 'npx tsx scripts/_probe-blank-origin-live.mts',  needServer: true },
  ...
  // 기본 담당자(2026-09-15 · 마이그 164) — 「일반관리」 고객이 담당 미선택으로 저장되면 설정된
  //   직원으로 채우고 「(기본)」을 붙인다. 위 형제와 **반대 방향**이라 나란히 둔다: 저기는
  //   미배정을 드러내고 여기는 메운다. 둘이 어긋나지 않는 이유가 바로 그 표식이다.
  // ⭐ 판정은 「채우는가」가 아니라 **「채우면 안 될 때 안 채우는가」**다 — 설정이 비었을 때·이미
  //   배정됐을 때·유형이 작동/종합일 때 셋. 채우기만 하면 빨간 「미배정」이라는 유일한 신호를
  //   잃어 문제를 고치는 게 아니라 **숨기는 것**이 된다.
  // ⭐ 표시 표면이 넷이라(요약·상세·목록·지역별 배정) **전수로 센다** — 하나만 빠져도 보는 자리에
  //   따라 「정식 배정」으로 읽힌다. 배정 권한이 있으면 드롭다운이 그려져 읽기 전용 라벨을 못 보므로
  //   선택 상자 옆 배지도 따로 단언한다.
  //   변이 프로브: `node scripts/_mutate-default-assignee.mjs` (12/12 빨강 실측 2026-09-15 —
  //   M9·M11이 「파일에 문자열만 있으면 초록」이던 사각을 드러내 분기 수·select 안쪽을 묻게 했다)
  { name: '기본 담당자 규칙',          cmd: 'npx tsx scripts/test-default-assignee.mts' },
  // 신규등록 순서 — 기본정보 → 건물·시설 → 관계인 → 소방계획서. 종전엔 등록 폼이 `?tab=plan`으로
  // **직행**시켜 가운데 두 탭을 통째로 건너뛰었다(그래서 용도가 비었다).
  // ⭐ 이 축의 실패는 **양방향**이다: 건너뛰는 것만큼 **다 채운 사용자를 붙잡는 것**도 실패다
  //   (사용자 요청이 "모두 채워지면 소방계획서로 가도 됩니다"였다) → 음성 단언을 짝으로 건다.
  // ⭐ 탭 ⚠ 배지와 온보딩 이동이 **같은 함수**를 써야 「⚠인데 직행시킨다」가 안 생긴다 —
  //   배선 단언이 tabDefs가 obState를 쓰는지를 본다(문자열 존재가 아니라).
  // 🚨 차단은 못 한다 — 실측 활성 305명 중 건물·관계인 완비는 10명(3.3%)뿐이라 상시 잠금은
  //   96.7%를 소방계획서에서 막는다(사용자 확정: 신규등록 흐름만 순서대로).
  ...
  // 점검 기간 — **종료일과 일수가 서로를 모르고 있었다**(2026-09-21 사용자 신고). 두 state가
  //   독립이라 종료일만 고치고 저장하면 일수는 `1`로 남았고, 실측에서 **다일 점검 3건 중 3건
  //   전부**가 그 상태였다(기간 3·4·8일 / 일수 1). 🚨 그 일수는 **별지 9호에 인쇄된다** —
  //   법정 서류 한 장에서 기간과 일수가 서로를 부정했다.
  //   양끝 포함 셈법(시작일이 1일차)은 총 이행기간·단계 마감일과 **같은 축**이다. 왕복 항등으로
  //   묶어 「+N」 하루 어긋남의 재발을 막는다(그 사고가 실제로 있었다).
  { name: '점검 기간 종료일↔일수(E2E)', cmd: 'npx tsx scripts/test-inspection-period.mts',   needServer: true },
  ...
  // ②로 들어가는 **문이 둘**이다(단계 줄 [배치확인서] · 패널 하단 [N단계로 이동]) — 2026-09-21 실측에서
  //   하단 문만 복귀 주소에 `insp`를 안 실어, **그 문으로 들어간 사용자만** 돌아왔을 때 우측바가
  //   닫혀 있었다. 위 왕복 검사는 `.first()`로 ①만 보고 소스도 모양만 봐서 이 갈림을 못 봤다.
  //   핵심은 「어딘가에서 insp를 쓰는가」가 아니라 **두 문이 같은 규약인가**다.
  //   ⚠ 표본을 **직접 만든다** — 달력 UI 구동은 그 달 표본에 의존해 흔들린다(위 검사 주석 참조).
  { name: '달력 ②왕복 — 문 둘(E2E)',    cmd: 'npx tsx scripts/test-calendar-step2-back.mts',          needServer: true },
  ...
  // 별지 9호 2쪽 인쇄 칸 4개(건축허가일·사용승인일·건축면적·높이) 라벨이 **비었을 때만** 빨강인가.
  // 🎯 핵심은 「빨갛게 했는가」가 아니라 **「채우면 빨강이 사라지는가」**다 — 사용자 요청은 문자
  //    그대로는 「상시 빨강」이었지만 실측(활성 311동)에서 네 칸을 다 채운 동이 6동뿐이라, 상시면
  //    그 6동에 거짓 경고가 되고 나머지엔 영구 소음이 된다. 그래서 대조군(채운 판)이 계약의 절반이다.
  { name: '별지9호 공란 라벨 빨강(E2E)',  cmd: 'node scripts/_verify-a9-blank-labels.mjs',           needServer: true },
  // 법정 기본(3순위) 이행기간이 **PDF에서만 안 깔리던** 축(2026-09-15). 관문 술어가 갈라져 있었다:
  //   PDF=plannedCount(폐지된 action_plan|start|end) vs 엑셀=defectRows(X 응답+불량행).
  // 🚨 실측 3회차가 갈라져 있었고, 제품 대조군(수리 되돌림)이 그 자리에서 **공란**을 재현했다.
  { name: '법정기간 PDF 축(E2E)',         cmd: 'node scripts/_verify-legal-period-pdf-axis.mjs',     needServer: true },
  ...
  // 목록 문서 바로가기(2026-09-16 사용자 요청) — 고객 관리·점검 업무 **첫페이지**에서 소방계획서
  //   엑셀·PDF와 결과보고서 엑셀을 바로 받는다. 종전엔 상세 안쪽에만 있었다.
  // 🎯 핵심 단언은 개수가 아니라 **행-문서 대응**이다 — 전 행이 같은 고객 문서를 가리켜도
  //   「행 수 = 버튼 수」는 초록이다(M2가 그 실수를 재현한다).
  // 🎯 「글씨로」는 사용자 지시라 라벨 글자와 **svg 부재**를 함께 묻는다 — 아이콘을 남긴 채
  //   글씨만 더해도 앞의 단언은 통과한다(M1).
  // 🚨 권한 음성 검사는 **일부러 없다**: 역할 셋이 모두 customer_manage·inspection_register를
  //   가져 «잡힐 수 없는 변이»다(동등 변이). 대신 C절이 화면·라우트 술어의 **일치**를 본다(M4).
  //   변이 프로브: `node scripts/_mutate-doc-shortcuts.mjs` (4/4 빨강 실측 2026-09-16)
  { name: '목록 문서 바로가기(E2E)',    cmd: 'npx tsx scripts/test-doc-shortcuts.mts', needServer: true },
]
